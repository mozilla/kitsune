...
class KnowledgeBaseNewArticle(Base):
    ...
    _first_article_topics_panel = (
        By.CSS_SELECTOR,
        "#ui-accordion-accordion-panel-0.ui-accordion-content-active",
    )
    _article_topic_expander_locator = (By.CSS_SELECTOR, "#accordion li strong")
    _first_article_topic_locator = (
        By.CSS_SELECTOR,
        "#ui-accordion-accordion-panel-0 input[name=topics]",
    )
    ...
    def check_first_article_topic(self):
        ...
        WebDriverWait(self.selenium, self.timeout).until(
            lambda s: "overflow:hidden"
            not in s.find_element(*self._first_article_topics_panel).get_attribute("style")
        )
