...
BZ_RESOLUTIONS = ["", "FIXED", "INVALID", "WONTFIX", "DUPLICATE", "WORKSFORME", "DUPLICATE"]
BZ_FIELDS = [
    "id",
    "status",
    "resolution",
    ...
    for bug in bugs:
        for item in bug.get("history", []):
            for change in item.get("changes", []):
                ...
                if (
                    change["field_name"] == "status_whiteboard"
                    and removed["s"] != sprint
                    and added["s"] == sprint
                ):

                    history.append((item["when"], bug, item["who"], removed["s"], added["s"]))
...
def sprint_timeline(bugs, sprint):
    ...
    for when, bug, who, removed, added in history:
        ...
        timeline.append((when, bug["id"], who, removed, added, reason))
