...
def wrap(text, indent="    ", subsequent="    "):
    ...
    text = [
        textwrap.fill(
            part, expand_tabs=True, initial_indent=indent, subsequent_indent=subsequent
        )
        for part in text
    ]
    return "\n\n".join(text)
...
def print_bugzilla_stats(from_date, to_date):
    ...
    print("")
    commenters = sorted(
        list(commenters.items()), reverse=True, key=lambda item: item[1]
    )
