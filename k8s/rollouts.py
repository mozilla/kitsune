...
def rollout_status(ctx, deployment_name):
    ...
    ctx.run("{} -n {} rollout status deploy {}".format(get_kubectl(), namespace, deployment_name))


def rollback(ctx, deployment_name):
    namespace = ctx.config["kubernetes"]["namespace"]
    ctx.run("{} -n {} rollout undo deploy {}".format(get_kubectl(), namespace, deployment_name))
