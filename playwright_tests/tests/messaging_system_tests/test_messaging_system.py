...
class TestMessagingSystem(TestUtilities):
    ...
    def test_there_are_no_messages_here_text_is_displayed_when_no_messages_are_available(self):
        ...
        check.equal(
            ...
            f"Received: "
            f"{self.sumo_pages.inbox_page._get_text_of_inbox_no_message_header()}",
        )
        ...
        check.equal(
            ...
            f"Received: "
            f"{self.sumo_pages.sent_message_page._get_sent_messages_no_message_text()}",
        )
    ...
    def test_new_message_field_validation(self):
        ...
        check.equal(
            ...
            f"Received: {self.sumo_pages.new_message_page._get_characters_remaining_text()}",
        )
    ...
    def test_new_message_preview(self):
        ...
        check.equal(
            ...
            f"Received: "
            f"{self.sumo_pages.new_message_page._get_text_of_test_data_first_paragraph_text()}",
        )

        check.equal(
            self.sumo_pages.new_message_page._get_text_of_test_data_first_p_strong_text(),
            ...
            f"Received: "
            f"{self.sumo_pages.new_message_page._get_text_of_test_data_first_p_strong_text()}",
        )

        check.equal(
            self.sumo_pages.new_message_page._get_text_of_test_data_first_p_italic_text(),
            ...
            f"Received: "
            f"{self.sumo_pages.new_message_page._get_text_of_test_data_first_p_italic_text()}",
        )
        ...
        check.equal(
            self.sumo_pages.new_message_page._get_text_of_numbered_list_items(),
            numbered_list_items,
            f"Wrong data displayed in the numbered list. Expected {numbered_list_items} "
            f"Received: {self.sumo_pages.new_message_page._get_text_of_numbered_list_items()}",
        )

        check.equal(
            self.sumo_pages.new_message_page._get_text_of_bulleted_list_items(),
            bulleted_list_items,
            f"Wrong data displayed in the numbered list. Expected {bulleted_list_items} "
            f"Received: {self.sumo_pages.new_message_page._get_text_of_numbered_list_items()}",
        )
        expect(
            self.sumo_pages.new_message_page._new_message_preview_external_link_test_data_element()
        ).to_be_visible()
