...
def test_kb_restrict_visibility(page: Page, is_template):
    ...
    with allure.step("Signing in with an admin account and removing all restricted groups"):
        ...
        sumo_pages.edit_article_metadata_flow.remove_a_restricted_visibility_group(group_name='')
...
@pytest.mark.kbRestrictedVisibility
@pytest.mark.parametrize("is_template", [False, True])
def test_kb_restricted_visibility_discussion(page: Page, is_template):
    utilities = Utilities(page)
    sumo_pages = SumoPages(page)
    ...
    with check, allure.step("Verifying that the the kb article is displayed for admin users"):
        expect(sumo_pages.article_discussions_page
               .is_title_for_article_discussion_displayed(thread['thread_title'])).to_be_visible()
...
# C2466533
@pytest.mark.kbRestrictedVisibility
def test_kb_restricted_visibility_in_topics_page(page: Page):
    utilities = Utilities(page)
    sumo_pages = SumoPages(page)
