...
def test_deleted_user_is_removed_from_groups(page: Page, create_user_factory):
    ...
    with check, allure.step("Verifying that the test user is displayed in both group leaders and "
    ...
    with check, allure.step("Navigating back to the test group and verifying that: "
                            "1. The deleted username is no longer displayed inside the group "
                            "leaders or members. "
