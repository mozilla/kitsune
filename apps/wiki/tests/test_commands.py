...
class InitContributorTests(TestCase):
    ...
    def test_two_approved_revision(self):
        ...
        d = r1.document
        u2 = user(save=True)
        r2 = revision(save=True, is_approved=True, creator=u2, document=d)
    ...
    def test_two_approved_with_rejected_and_unreviewed_between(self):
        ...
        d = r1.document
        u2 = user(save=True)
        r2 = revision(save=True, creator=u2, document=d)
        u3 = user(save=True)
        r3 = revision(save=True, reviewed=datetime.now(), creator=u3)
        u4 = user(save=True)
        r4 = revision(save=True, is_approved=True, creator=u4, document=d)
        d.contributors.clear()
        _init_contributors()
        assert u1 in d.contributors.all()
        assert u2 in d.contributors.all()
