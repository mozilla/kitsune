...
def threads(request, forum_slug):
    ...
    threads_ = paginate(
        request, threads_, per_page=constants.THREADS_PER_PAGE, count=count
    )

    feed_urls = (
        (reverse("forums.threads.feed", args=[forum_slug]), ThreadsFeed().title(forum)),
    )
...
def posts(request, forum_slug, thread_id, form=None, post_preview=None, is_reply=False):
    ...
    feed_urls = (
        (
            reverse(
                "forums.posts.feed",
                kwargs={"forum_slug": forum_slug, "thread_id": thread_id},
            ),
    ...
    if form.is_valid():
        if "preview" in request.POST:
            ...
            post = thread.new_post(
                author=request.user, content=form.cleaned_data["content"]
            )
            post.save()
...
def delete_thread(request, forum_slug, thread_id):
    ...
    if request.method == "GET":
        ...
        return render(
            request,
            "forums/confirm_thread_delete.html",
...
def move_thread(request, forum_slug, thread_id):
    ...
    log.warning(
        "User %s is moving thread with id=%s to forum with id=%s"
