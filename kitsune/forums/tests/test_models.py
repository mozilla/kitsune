...
class ForumModelTestCase(ForumTestCase):
    ...
    def test_post_absolute_url(self):
        ...
        url = reverse(
            "forums.posts",
            kwargs={"forum_slug": p1.thread.forum.slug, "thread_id": p1.thread.id},
        )
        eq_(urlparams(url, hash="post-%s" % p1.id), p1.get_absolute_url())

        url = reverse(
            "forums.posts",
            kwargs={"forum_slug": p2.thread.forum.slug, "thread_id": p2.thread.id},
        )
    ...
    def test_access_restriction(self):
        ...
        PermissionFactory(
            codename="forums_forum.view_in_forum", content_type=ct, object_id=f.id
        )
        PermissionFactory(
