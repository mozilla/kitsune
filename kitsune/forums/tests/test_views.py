...
class ThreadAuthorityPermissionsTests(ForumTestCase):
    ...
    def test_watch_forum_without_permission(self):
        ...
        response = self.client.post(
            reverse("forums.watch_forum", args=[rforum.slug]),
            {"watch": "yes"},
            follow=False,
        )
        eq_(404, response.status_code)
...
class ThreadPermissionsTests(ForumTestCase):
    ...
    def test_edit_locked_thread_403(self):
        ...
        self.client.login(username=u.username, password="testpass")
        response = get(
            self.client, "forums.edit_thread", args=[locked.forum.slug, locked.id]
        )
        eq_(403, response.status_code)
    ...
    def test_post_delete_403(self):
        ...
        self.client.login(username=u.username, password="testpass")
        response = get(
            self.client, "forums.delete_post", args=[t.forum.slug, t.id, p.id]
        )
        eq_(403, response.status_code)
