...
class ForumTestPermissions(ForumTestCase):
    ...
    def setUp(self):
        ...
        for name in permission_names:
            PermissionFactory(
                codename=name, content_type=ct, object_id=f.id, group=self.group
            )
    ...
    def test_has_perm_thread_edit(self):
        ...
        assert not has_perm(
            self.context, "forums_forum.thread_edit_forum", self.forum_2
        )
    ...
    def test_has_perm_thread_delete(self):
        ...
        assert not has_perm(
            self.context, "forums_forum.thread_delete_forum", self.forum_2
        )
    ...
    def test_has_perm_thread_sticky(self):
        ...
        assert not has_perm(
            self.context, "forums_forum.thread_sticky_forum", self.forum_2
        )
    ...
    def test_has_perm_thread_locked(self):
        ...
        self.context["request"].user = u
        assert not has_perm(
            self.context, "forums_forum.thread_locked_forum", self.forum_1
        )
    ...
    def test_has_perm_post_delete(self):
        ...
        assert not has_perm(
            self.context, "forums_forum.post_delete_forum", self.forum_2
        )
    ...
    def test_no_perm_thread_delete(self):
        ...
        assert not has_perm(
            self.context, "forums_forum.thread_delete_forum", self.forum_1
        )
        assert not has_perm(
            self.context, "forums_forum.thread_delete_forum", self.forum_2
        )
