...
class Product(ModelBase):
    ...
    image_alternate = models.ImageField(
        ...
        blank=True,
        max_length=settings.MAX_FILEPATH_LENGTH,
        help_text=(u"Used everywhere except the home " "page. Must be 96x96."),
    )
    image_offset = models.IntegerField(default=None, null=True, editable=False)
    image_cachebuster = models.CharField(
        max_length=32, default=None, null=True, editable=False
    )
    ...
    def image_url(self):
        ...
        return os.path.join(
            settings.STATIC_URL, "products", "img", "product_placeholder.png"
        )
...
class Topic(ModelBase):
    ...
    parent = models.ForeignKey("self", related_name="subtopics", null=True, blank=True)
    ...
    in_aaq = models.BooleanField(
        default=False,
        help_text=_lazy(u"Whether this topic is shown to users in the AAQ or not."),
    )
    ...
    def image_url(self):
        if self.image:
            return self.image.url
        return os.path.join(
            settings.STATIC_URL, "products", "img", "topic_placeholder.png"
        )
