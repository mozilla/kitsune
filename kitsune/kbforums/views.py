...
def threads(request, document_slug):
    ...
    feed_urls = (
        (
            reverse("wiki.discuss.threads.feed", args=[document_slug]),
...
def new_thread(request, document_slug):
    ...
    if request.method == "GET":
        form = NewThreadForm()
        return render(
            request, "kbforums/new_thread.html", {"form": form, "document": doc}
        )
    ...
    if form.is_valid():
        if "preview" in request.POST:
            ...
            thread = doc.thread_set.create(
                creator=request.user, title=form.cleaned_data["title"]
            )
            thread.save()
            statsd.incr("kbforums.thread")
            post = thread.new_post(
                creator=request.user, content=form.cleaned_data["content"]
            )
            post.save()
...
def edit_thread(request, document_slug, thread_id):
    ...
    if form.is_valid():
        log.warning(
            "User %s is editing KB thread with id=%s" % (request.user, thread.id)
        )
...
def delete_thread(request, document_slug, thread_id):
    ...
    if request.method == "GET":
        ...
        return render(
            request,
            "kbforums/confirm_thread_delete.html",
...
def edit_post(request, document_slug, thread_id, post_id):
    ...
    if form.is_valid():
        ...
        if "preview" in request.POST:
            ...
            log.warning(
                "User %s is editing KB post with id=%s" % (request.user, post.id)
            )
            post.save()
...
def post_preview_async(request, document_slug):
    ...
    return render(
        request, "kbforums/includes/post_preview.html", {"post_preview": post}
    )
