...
class NotificationsTests(KBForumTestCase):
    ...
    def test_watch_forum_then_new_post(self, get_current):
        ...
        p = Post.objects.all().order_by("-id")[0]
        attrs_eq(
            mail.outbox[0], to=[u.email], subject="Re: an article title - Sticky Thread"
        )
        starts_with(
            mail.outbox[0].body,
            REPLY_EMAIL
    ...
    def test_autowatch_new_thread(self, get_current):
        ...
        s = Setting.objects.create(
            user=u, name="kbforums_watch_new_thread", value="False"
        )
    ...
    def test_autowatch_reply(self, get_current):
        ...
        self.client.login(username=u.username, password="testpass")
        s = Setting.objects.create(
            user=u, name="kbforums_watch_after_reply", value="True"
        )
