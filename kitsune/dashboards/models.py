...
from kitsune.dashboards import (
    LAST_7_DAYS,
    LAST_30_DAYS,
    LAST_90_DAYS,
    ALL_TIME,
    ...
    def reload_period_from_analytics(cls, period, verbose=False):
        ...
        if counts:
            ...
            for doc_id, visits in counts.iteritems():
                cls.objects.create(
                    document=Document(pk=doc_id), visits=visits, period=period
                )
        else:
...
METRIC_CODE_CHOICES = (
    ...
    (L10N_ACTIVE_CONTRIBUTORS_CODE, _lazy(u"Monthly Active Contributors")),
)
