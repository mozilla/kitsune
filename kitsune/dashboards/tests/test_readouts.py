    ...
    def titles(self, locale=None, product=None):
        ...
        return [
            ...
            for row in self.readout(
                MockRequest(), locale=locale, product=product
            ).rows()
        ]
...
class L10NOverviewTests(TestCase):
    ...
    def test_miscounting_archived(self):
        ...
        trans_rev1 = ApprovedRevisionFactory(
            document=parent1, is_ready_for_localization=True
        )
        ApprovedRevisionFactory(document=translation1, based_on=trans_rev1)
        trans_rev2 = ApprovedRevisionFactory(
            document=parent2, is_ready_for_localization=True
        )
...
class HowToContributeTests(ReadoutTestCase):
    ...
    def test_only_how_to_contribute(self):
        ...
        d2 = DocumentFactory(
            title="How To", category=HOW_TO_CONTRIBUTE_CATEGORY, products=[p]
        )
...
class AdministrationTests(ReadoutTestCase):
    ...
    def test_only_how_to_contribute(self):
        ...
        d1 = DocumentFactory(products=[p])
        d2 = DocumentFactory(
            title="Admin", category=ADMINISTRATION_CATEGORY, products=[p]
        )
...
class TemplateTranslationsTests(ReadoutTestCase):
    ...
    def test_untranslated(self):
        ...
        d = TemplateDocumentFactory()
        untranslated = ApprovedRevisionFactory(
            document=d, is_ready_for_localization=True
        )
        row = self.row()
        eq_(row["title"], untranslated.document.title)
        eq_(unicode(row["status"]), "Translation Needed")
...
class CannedResponsesTests(ReadoutTestCase):
    ...
    def test_translation_state(self):
        ...
        eng_rev = ApprovedRevisionFactory(
            is_ready_for_localization=True, document=eng_doc
        )

        eq_("untranslated", self.row()["status_class"])

        # Now translate it, but don't approve
        de_doc = DocumentFactory(
            category=CANNED_RESPONSES_CATEGORY, parent=eng_doc, locale="de"
        )
