...
SIGNIFICANCE_STATUSES = {
    MEDIUM_SIGNIFICANCE: (_lazy(u"Update Needed"), "wiki.edit_document", "update"),
    MAJOR_SIGNIFICANCE: (
        _lazy(u"Immediate Update Needed"),
        "wiki.edit_document",
        "out-of-date",
    ),
}
...
def kb_overview_rows(mode=None, max=None, locale=None, product=None, category=None):
    ...
    for d in docs:
        data = {
            "url": reverse(
                "wiki.document", args=[d.slug], locale=settings.WIKI_DEFAULT_LANGUAGE
            ),
            "trans_url": reverse(
        ...
        if d.current_revision:
            unapproved_revs = d.revisions.filter(
                reviewed=None, id__gt=d.current_revision.id
            )[:1]
        else:
...
class MostVisitedDefaultLanguageReadout(Readout):
    ...
    def _format_row(self, (slug, title, visits, num_unreviewed)):
        ...
        return dict(
            ...
            visits=visits,
            status=status,
            status_url=reverse(view_name, args=[slug], locale=self.locale)
            if view_name
            else "",
        )
...
class CategoryReadout(Readout):
    ...
    def _format_row(
        self, (slug, title, visits, needs_changes, needs_review, unready_for_l10n)
    ):
        if needs_review:
            status, view_name, dummy = REVIEW_STATUSES[needs_review]
        elif needs_changes:
...
class MostVisitedTranslationsReadout(MostVisitedDefaultLanguageReadout):
    ...
    def _query_and_params(self, max):
        ...
        extra_where = (
            "AND NOT engdoc.category IN (" + ", ".join(ignore_categories) + ") "
        )
        ...
        return (
            ...
            + NEEDS_REVIEW
            + most_visited_translation_from(
                extra_joins=extra_joins, extra_where=extra_where
            )
...
class TemplateTranslationsReadout(Readout):
    ...
    def _format_row(
        self, (eng_slug, eng_title, slug, title, significance, needs_review)
    ):
    ...
    def sort_and_truncate(self, rows, max):
        ...
        # first in ordering.
        rows.sort(
            key=lambda row: (row["status"] == u"", row["status_class"], row["title"])
        )
        return rows[:max]
...
class UnreviewedReadout(Readout):
    ...
    def _query_and_params(self, max):
        english_id = (
            "id" if self.locale == settings.WIKI_DEFAULT_LANGUAGE else "parent_id"
        )
...
class UnreadyForLocalizationReadout(Readout):
    ...
    description = _lazy(
        u"Articles which have approved revisions newer than "
        u"the latest ready-for-localization one"
    )
    # No short_title; the Contributors dash lacks an Overview readout
    details_link_text = _lazy(
        u"All articles with changes not ready for " u"localization..."
    )
    slug = "unready"
