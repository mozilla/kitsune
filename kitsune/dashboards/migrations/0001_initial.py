...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('dashboards_wikidocumentvisits', (
        ...
        db.send_create_signal('dashboards', ['WikiDocumentVisits'])

        # Adding unique constraint on 'WikiDocumentVisits', fields ['period', 'document']
        db.create_unique('dashboards_wikidocumentvisits', ['period', 'document_id'])

        # Adding model 'WikiMetric'
        db.create_table('dashboards_wikimetric', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('code', self.gf('django.db.models.fields.CharField')(max_length=255, db_index=True)),
            ('locale', self.gf('kitsune.sumo.models.LocaleField')(default='en-US', max_length=7, null=True, db_index=True, blank=True)),
        ...
        db.send_create_signal('dashboards', ['WikiMetric'])

        # Adding unique constraint on 'WikiMetric', fields ['code', 'product', 'locale', 'date']
        db.create_unique('dashboards_wikimetric', ['code', 'product_id', 'locale', 'date'])


    def backwards(self, orm):
        # Removing unique constraint on 'WikiMetric', fields ['code', 'product', 'locale', 'date']
        db.delete_unique('dashboards_wikimetric', ['code', 'product_id', 'locale', 'date'])

        # Removing unique constraint on 'WikiDocumentVisits', fields ['period', 'document']
        db.delete_unique('dashboards_wikidocumentvisits', ['period', 'document_id'])

        # Deleting model 'WikiDocumentVisits'
        db.delete_table('dashboards_wikidocumentvisits')

        # Deleting model 'WikiMetric'
        db.delete_table('dashboards_wikimetric')
