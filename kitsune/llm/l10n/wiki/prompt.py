...
# Instructions for translating special strings
- Each of the following instructions describes how to translate special strings that may be present within the {{ source_language }} text that you are translating.
- **Strictly obey** all of these instructions when freshly translating {{ source_language }} text.

1. **Preserve unchanged** any of the following strings:
    - `Anonym`
    - `Bugzilla`
    - `ESR`
...
2. **Preserve unchanged** any string that case-sensitively matches the following regular expression:

    ```python
    r"\{(for|key|filepath) .*?\}"
    ```

3. For any string that case-sensitively matches the following regular expression:

    ```python
    r"\{(?:button|menu|pref) (?P<description>.*?)\}"
    ```

    - Translate only the text matched by the named group `description` (while obeying instruction #1 above), and **preserve the rest of the string unchanged**.

4. Each `wiki-hook` must be translated as follows:
...
5. Each `wiki-article-link` must be translated as follows:
    - First, check if the `wiki-article-link` is a key within the `prior-translation-wiki-map`.
    - If it is a key within the `prior-translation-wiki-map`, use its value from the `prior-translation-wiki-map` as its translation.
    - If it is **not** a key within the `prior-translation-wiki-map`, translate only the text matched by the named group `description` (while obeying instruction #1 above), and **preserve the rest unchanged**.

6. Each `wiki-external-link` must be translated as follows:
...
4. For each part that is different, **freshly translate** that part while **strictly obeying** the `Instructions for translating special strings`.
5. **Review** your translation and its {{ source_language }} source text to ensure **strict compliance with all instructions**, including the `Instructions for translating special strings`, and make any corrections as needed.
6. In your response, include your translation and an explanation describing what you did for each step.
