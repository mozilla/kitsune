...
class DocumentTests(TestCase):
    ...
    def _test_remembering_setter_unsaved(self, field):
        ...
        assert not hasattr(d, old_field), (
            "Doc shouldn't have %s until it's saved." % old_field
        )
...
class FromUrlTests(TestCase):
    ...
    def test_redirect_to_translated_document(self):
        ...
        d_tr = DocumentFactory(
            locale="tr", title=u"Google Chrome'u nasıl silerim?", parent=d_en
        )
...
class LocalizableOrLatestRevisionTests(TestCase):
    ...
    def test_multiple_ready(self):
        ...
        r2 = ApprovedRevisionFactory(
            document=r1.document, is_ready_for_localization=True
        )
    ...
    def test_ready_over_recent(self):
        ...
        ready = ApprovedRevisionFactory(
            is_approved=True, is_ready_for_localization=True
        )
        ApprovedRevisionFactory(
            document=ready.document, is_ready_for_localization=False
        )
    ...
    def test_latest_rejected_if_none_unreviewed(self):
        ...
        eq_(
            rejected,
            rejected.document.localizable_or_latest_revision(include_rejected=True),
        )
    ...
    def test_non_localizable(self):
        ...
        r2 = ApprovedRevisionFactory(
            document=r1.document, is_ready_for_localization=False
        )
...
class RedirectCreationTests(TestCase):
    ...
    def _test_collision_avoidance(self, attr, other_attr, template):
        ...
        kwargs = {
            other_attr: template % {"old": getattr(self.d, other_attr), "number": 1}
        }
...
class RevisionTests(TestCase):
    ...
    def test_correct_ready_for_localization_if_insignificant(self):
        ...
        r = ApprovedRevisionFactory(
            is_ready_for_localization=True, significance=TYPO_SIGNIFICANCE
        )
        r.clean()
        assert not r.is_ready_for_localization
