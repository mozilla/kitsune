...
class TestWikiVideo(TestCase):
    ...
    def test_video_english(self):
        ...
        # The exact HTML format varies across parser versions (html5lib,
        # libxml2, justhtml). Verify the structural content instead.
        video_html = doc("video").html()
        self.assertIn('src="{}"'.format(v.webm.url), video_html)
        self.assertIn('type="video/webm"', video_html)
        self.assertIn('src="{}"'.format(v.ogv.url), video_html)
        self.assertIn('type="video/ogg"', video_html)
