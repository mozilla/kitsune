...
def doc_parse_markup(content, markup):
    ...
    _, _, p = doc_rev_parser(
        content, TEMPLATE_TITLE_PREFIX + "test", category=TEMPLATES_CATEGORY
    )
...
class SimpleSyntaxTestCase(TestCase):
    ...
    def test_key_inline(self):
        ...
        eq_(1, len(doc("p")))
        eq_(
            u'<span class="key">Cmd</span> + <span class="key">Shift</span>'
            u' + <span class="key">Q</span>',
            doc.html().replace("\n", ""),
        )
...
class TestWikiTemplate(TestCase):
    ...
    def test_template_does_not_exist(self):
        ...
        eq_(
            'The template "test" does not exist or has no approved revision.',
            doc.text(),
        )
    ...
    def test_template_args_inline_wiki_markup(self):
        ...
        eq_(
            "<p/><p><strong>one</strong></p><p><em>two</em></p><p/>",
            doc.html().replace("\n", ""),
        )
    ...
    def test_direct_recursion(self):
        ...
        ApprovedRevisionFactory.create_batch(
            2,
            document=d,
            content="Fine [[{}Boo]] Fellows".format(TEMPLATE_TITLE_PREFIX),
        )
...
class TestWikiVideo(TestCase):
    ...
    def test_video_modal(self):
        ...
        v = VideoFactory()
        replacement = (
            '<img class="video-thumbnail" src="%s"/>' % v.thumbnail_url_if_set()
        )
...
class ForParserTests(TestCase):
    ...
    def test_early_close(self):
        ...
        its closer is early."""
        balanced_eq(
            "<div><for><p>One</p></for></div>", "<div><for><p>One</for></for></p></div>"
        )
...
class WhatLinksHereTests(TestCase):
    ...
    def test_unicode(self):
        ...
        ApprovedRevisionFactory(
            document=d2, content=u"What do you think about [[\u03C0]]?"
        )
...
class TestLazyWikiImageTags(TestCase):
    ...
    def test_simple(self):
        ...
        doc = pq(
            self.p.parse("[[Image:test.jpg]]", locale=settings.WIKI_DEFAULT_LANGUAGE)
        )
        img = doc("img")
