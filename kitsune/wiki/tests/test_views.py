...
from kitsune.wiki.views import (
    _document_lock_check,
    _document_lock_clear,
...
class RedirectTests(TestCase):
    ...
    def test_redirect_suppression(self):
        ...
        response = self.client.get(
            redirect.get_absolute_url() + "?redirect=no", follow=True
        )
...
class DocumentEditingTests(TestCase):
    ...
    def test_retitling_template(self):
        ...
        data = new_document_data()
        data.update(
            {"title": new_title, "category": d.category, "slug": d.slug, "form": "doc"}
        )
        ...
        assert (
            "Documents in the Template category must have titles that start with "
            "&#34;Template:&#34;." in res.content
        )
    ...
    def test_removing_template_category(self):
        ...
        # This message gets HTML encoded.
        assert (
            "Documents with titles that start with &#34;Template:&#34; must be in the "
    ...
    def test_new_document_slugs(self, get_current):
        ...
        self.assertRedirects(
            response,
            reverse("wiki.document_revisions", args=[data["slug"]], locale="en-US"),
        )
    ...
    def test_localized_based_on(self):
        ...
        fr_r = RevisionFactory(
            document=fr_d, based_on=en_r, keywords="oui", summary="lipsum"
        )
        url = reverse(
            "wiki.new_revision_based_on", locale="fr", args=(fr_d.slug, fr_r.pk,)
        )
        response = self.client.get(url)
    ...
    def test_needs_change(self):
        ...
        data = new_document_data()
        data.update(
            {"needs_change": True, "needs_change_comment": comment, "form": "doc"}
        )
    ...
    def test_draft_revision_view(self):
        ...
        obj = DraftRevision.objects.get(
            creator=self.u, document=rev.document, locale="bn"
        )
    ...
    def test_draft_revision_many_times(self):
        ...
        obj = DraftRevision.objects.filter(
            creator=self.u, document=rev.document, locale="bn"
        )
...
class VoteTests(TestCase):
    ...
    def test_source(self):
        ...
        self.client.post(
            url, {"revision_id": rev.id, "helpful": True, "source": "test",}
        )
    ...
    def test_rate_limiting(self):
        ...
        for i in range(13):
            rev = RevisionFactory()
            url = reverse(
                "wiki.document_vote", kwargs={"document_slug": rev.document.slug}
            )
...
class FallbackSystem(TestCase):
    ...
    def get_data_from_translated_document(
        self, header, create_doc_locale, req_doc_locale
    ):
