...
class Document(
    ...
    current_revision = models.ForeignKey(
        "Revision", null=True, related_name="current_for+"
    )
    ...
    parent = models.ForeignKey(
        "self", related_name="translations", null=True, blank=True
    )
    ...
    def _clean_is_localizable(self):
        ...
        if self.pk and not self.is_localizable and self.translations.exists():
            raise ValidationError(
                u'"{0}": document has {1} translations but is not localizable.'.format(
    ...
    def _clean_template_status(self):
        if self.category == TEMPLATES_CATEGORY and not self.title.startswith(
            ...
            raise ValidationError(
                _(
                    u"Documents in the Template category must have titles that "
        ...
        if (
            ...
            raise ValidationError(
                _(
                    u'Documents with titles that start with "{prefix}" must be in '
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            ...
            and (slug_changed or title_changed)
            and not treat_as_template
        ):
            try:
    ...
    def add_link_to(self, linked_to, kind):
        ...
        DocumentLink.objects.get_or_create(
            linked_from=self, linked_to=linked_to, kind=kind
        )
...
class DocumentMappingType(SearchMappingType):
    ...
    def extract_document(cls, obj_id, obj=None):
        if obj is None:
            model = cls.get_model()
            obj = model.objects.select_related("current_revision", "parent").get(
                pk=obj_id
            )
    ...
    def index(cls, document, **kwargs):
        ...
        if document["document_current_id"] is None or document[
            "document_content"
        ].startswith(REDIRECT_HTML):
...
class Revision(ModelBase, SearchMixin, AbstractRevision):
    ...
    reviewer = models.ForeignKey(User, related_name="reviewed_revisions", null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_approved and (
            not self.document.current_revision
            or self.document.current_revision.id < self.id
        ):
    ...
    def content_parsed(self):
        ...
        return wiki_to_html(
            self.content, locale=self.document.locale, doc_id=self.document.id
        )
...
class DocumentLink(ModelBase):
    ...
    linked_to = models.ForeignKey(Document, related_name="documentlink_from_set")
    linked_from = models.ForeignKey(Document, related_name="documentlink_to_set")
