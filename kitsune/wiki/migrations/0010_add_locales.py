...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Create the new Locales."""
        orm.Locale.objects.create(locale='et')
        orm.Locale.objects.create(locale='fy-NL')
        orm.Locale.objects.create(locale='gu-IN')
        orm.Locale.objects.create(locale='ur')

    def backwards(self, orm):
        """Remove the new Locales."""
        orm.Locale.objects.filter(locale='af').delete()
        orm.Locale.objects.filter(locale='fy-NL').delete()
        orm.Locale.objects.filter(locale='gu-IN').delete()
        orm.Locale.objects.filter(locale='ur').delete()
    ...
    models = {
        ...
        u'wiki.document': {
            ...
            'share_link': ('django.db.models.fields.CharField', [], {'default': "''", 'max_length': '24'}),
            'slug': ('django.db.models.fields.CharField', [], {'max_length': '255', 'db_index': 'True'}),
            'title': ('django.db.models.fields.CharField', [], {'max_length': '255', 'db_index': 'True'}),
