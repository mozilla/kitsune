...
def document(request, document_slug, document=None):
    ...
    if redirect_slug and redirect_locale:
        try:
            redirected_from = Document.objects.get(
                locale=redirect_locale, slug=redirect_slug
            )
        except Document.DoesNotExist:
            pass
    ...
    if len(document_topics) > 0:
        ...
        # Get the product
        breadcrumbs.append(
            (first_topic.product.get_absolute_url(), first_topic.product.title)
        )
    else:
...
def list_documents(request, category=None):
    ...
    return render(
        request, "wiki/list_documents.html", {"documents": docs, "category": category}
    )
...
def edit_document(request, document_slug, revision_id=None):
    ...
    try:
        ...
        # the article page.
            ...
            url = reverse(
                "wiki.translate", locale=request.LANGUAGE_CODE, args=[document_slug]
            )
            return HttpResponseRedirect(url)
    ...
    if doc.allows(user, "create_revision"):
        rev_form = RevisionForm(
            instance=rev, initial={"based_on": rev.id, "comment": ""}
        )
...
def document_revisions(request, document_slug, contributor_form=None):
    ...
    try:
        ...
        if translation:
            url = reverse(
                "wiki.document_revisions", args=[translation.slug], locale=locale
            )
            return HttpResponseRedirect(url)
        else:
            raise Http404
...
def review_revision(request, document_slug, revision_id):
    ...
    last_approved_date = getattr(
        doc.current_revision, "created", datetime.fromordinal(1)
    )
...
def select_locale(request, document_slug):
    ...
    translated_locales_code.extend(
        doc.translations.all().values_list("locale", flat=True)
    )
...
def translate(request, document_slug, revision_id=None):
    ...
    * translation is to the request.LANGUAGE_CODE
    ...
    if doc:
        ...
        more_updated_rev = (
            doc.revisions.filter(based_on__id__gt=based_on_rev.id)
            .order_by("based_on__id")
            .last()
        )
    else:
    ...
    fxos_l10n_warning = (
        "firefox-os" in product_slugs
        and request.LANGUAGE_CODE not in settings.FXOS_LANGUAGES
    )
...
def get_helpful_votes_async(request, document_slug):
    ...
    for flag in ImportantDate.objects.filter(
        date__gte=min_created, date__lte=max_created
    ):
        flag_data.append(
            {"x": int(time.mktime(flag.date.timetuple())), "text": _(flag.text)}
        )
...
def delete_revision(request, document_slug, revision_id):
    ...
    revision.delete()
    return HttpResponseRedirect(
        reverse("wiki.document_revisions", args=[document.slug])
    )
...
def delete_document(request, document_slug):
    ...
    if request.method == "GET":
        # Render the confirmation page
        return render(
            request, "wiki/confirm_document_delete.html", {"document": document}
        )

    # Handle confirm delete form POST
    log.warning(
        "User %s is deleting document: %s (id=%s)"
...
def _document_form_initial(document):
    ...
    return {
        ...
        "products": list(
            Product.objects.filter(document=document).values_list("id", flat=True)
        ),
...
def what_links_here(request, document_slug):
    ...
    c = {"document": doc, "relations": links}
