...
def generate_sampledata(options):
    ...
    for p in [firefox, mobile]:
        ...
        TopicFactory(
            product=p, title="Learn the Basics: get started", slug="get-started"
        )
        TopicFactory(
            product=p,
        ...
        TopicFactory(
            product=p, title="Customize controls, options and add-ons", slug="customize"
        )
        TopicFactory(
            product=p,
    ...
    for i in xrange(9):
        d = DocumentFactory(
            title="Sample Article %s" % str(i + 1),
