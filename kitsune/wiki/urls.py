...
document_patterns = [
    ...
    re_path(r"^steal-lock$", views.steal_lock, name="wiki.steal_lock"),
]
...
urlpatterns = [
    ...
    re_path(r"^save-draft$", views.draft_revision, name="wiki.draft_revision"),
    re_path(r"^category/(?P<category>\d+)$", views.list_documents, name="wiki.category"),
    # Document without trailing slash (for backwards compatibility)
    re_path(r"^(?P<document_slug>[^/]+)$", views.document, name="wiki.document"),
    # Document sub-paths with trailing slash
