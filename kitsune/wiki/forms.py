...
TITLE_SHORT = _lazy(
    u"The title is too short (%(show_value)s characters). "
    u"It must be at least %(limit_value)s characters."
)
TITLE_LONG = _lazy(
    u"Please keep the length of the title to %(limit_value)s "
...
SLUG_SHORT = _lazy(
    u"The slug is too short (%(show_value)s characters). "
    u"It must be at least %(limit_value)s characters."
)
SLUG_LONG = _lazy(
    u"Please keep the length of the slug to %(limit_value)s "
    u"characters or less. It is currently %(show_value)s "
    u"characters."
)
SUMMARY_REQUIRED = _lazy(u"Please provide a summary.")
SUMMARY_SHORT = _lazy(
    u"The summary is too short (%(show_value)s characters). "
    u"It must be at least %(limit_value)s characters."
)
SUMMARY_LONG = _lazy(
    u"Please keep the length of the summary to "
...
CONTENT_SHORT = _lazy(
    u"The content is too short (%(show_value)s characters). "
    u"It must be at least %(limit_value)s characters."
)
CONTENT_LONG = _lazy(
    u"Please keep the length of the content to "
    u"%(limit_value)s characters or less. It is currently "
    u"%(show_value)s characters."
)
COMMENT_LONG = _lazy(
    u"Please keep the length of the comment to "
    u"%(limit_value)s characters or less. It is currently "
    u"%(show_value)s characters."
)
...
class DocumentForm(forms.ModelForm):
    ...
    is_localizable = forms.BooleanField(
        initial=True, label=_lazy(u"Allow translations:"), required=False
    )

    is_archived = forms.BooleanField(label=_lazy(u"Obsolete:"), required=False)

    allow_discussion = forms.BooleanField(
        label=_lazy(u"Allow discussion on this article?"), initial=True, required=False
    )
    ...
    needs_change = forms.BooleanField(
        label=_lazy(u"Needs change:"), initial=False, required=False
    )

    needs_change_comment = forms.CharField(
        label=_lazy(u"Comment:"), widget=forms.Textarea(), required=False
    )
...
class RevisionForm(forms.ModelForm):
    ...
    expires = forms.DateField(label=_lazy(u"Expiry date:"), required=False)
...
class ReviewForm(forms.Form):
    ...
    is_ready_for_localization = forms.BooleanField(
        initial=False, label=_lazy(u"Ready for localization"), required=False
    )
...
class AddContributorForm(forms.Form):
    ...
    users = MultiUsernameField(
        widget=forms.TextInput(
            attrs={"placeholder": _lazy(u"username"), "class": "user-autocomplete"}
        )
    )


languages = [("", "Any")] + [
    (l[0], u"{1} ({0})".format(*l)) for l in settings.LANGUAGE_CHOICES
]
...
class RevisionFilterForm(forms.Form):
    ...
    locale = forms.ChoiceField(
        label=_lazy(u"Locale:"), choices=languages, required=False
    )
