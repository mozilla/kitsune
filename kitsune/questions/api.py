...
class QuestionSerializer(serializers.ModelSerializer):
    ...
    metadata = QuestionMetaDataSerializer(
        source="metadata_set", read_only=True, many=True
    )
...
class QuestionFilter(django_filters.FilterSet):
    ...
    def filter_solved_by(self, queryset, name, value):
        question_user_solved = Question.objects.filter(
            solution__creator__username=value
        ).values("id")
...
class QuestionViewSet(viewsets.ModelViewSet):
    ...
    def follow(self, request, pk=None):
        ...
        actstream.actions.follow(
            request.user, question, actor_only=False, send_action=False
        )
    ...
    def delete_metadata(self, request, pk=None):
        ...
        try:
            meta = QuestionMetaData.objects.get(
                question=question, name=request.data["name"]
            )
            meta.delete()
            return Response(status=status.HTTP_204_NO_CONTENT)
...
class AnswerSerializer(serializers.ModelSerializer):
    ...
    def get_updated_by(self, obj):
        updated_by = (
            Profile.objects.get(user=obj.updated_by) if obj.updated_by else None
        )
...
class AnswerViewSet(viewsets.ModelViewSet):
    ...
    def helpful(self, request, pk=None):
        ...
        num_helpful_votes = AnswerVote.objects.filter(
            answer=answer, helpful=True
        ).count()
        num_unhelpful_votes = AnswerVote.objects.filter(
            answer=answer, helpful=False
        ).count()
        return Response(
            {
