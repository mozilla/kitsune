...
class Question(ModelBase, BigVocabTaggableMixin, SearchMixin):
    ...
    def clear_mutable_metadata(self):
        ...
        self.metadata_set.exclude(
            name__in=["useragent", "product", "category"]
        ).delete()
        self._metadata = None
    ...
    def auto_tag(self):
        ...
        """
        to_add = self.product_config.get("tags", []) + self.category_config.get(
            "tags", []
        )
...
class QuestionMappingType(SearchMappingType):
    ...
    def extract_document(cls, obj_id, obj=None):
        ...
        d["question_tag"] = list(
            TaggedItem.tags_for(Question, Question(pk=obj_id)).values_list(
                "name", flat=True
            )
        )
...
class QuestionVisits(ModelBase):
    ...
    def reload_from_analytics(cls, verbose=False):
        ...
        if counts:
            ...
            log.warning(
                "Google Analytics returned no interesting data,"
...
class Answer(ModelBase, SearchMixin):
    ...
    def save(self, update=True, no_notify=False, *args, **kwargs):
        ...
        latest = Answer.objects.filter(question=self.question, is_spam=False).order_by(
            "-created"
        )[:1]
...
class AnswerMetricsMappingType(SearchMappingType):
    ...
    def extract_document(cls, obj_id, obj=None):
        ...
        # doesn't change much, so this is probably ok.
        url = reverse(
            "questions.details", kwargs={"question_id": obj_dict["question_id"]}
        )
        ...
        if user.username != instance.username:
            questions = (
                Question.objects.filter(
                    Q(creator=instance) | Q(answers__creator=instance)
                )
                .only("id")
                .distinct()
            )
