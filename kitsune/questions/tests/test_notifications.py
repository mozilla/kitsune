...
from kitsune.questions.events import (
    QuestionReplyEvent,
    QuestionSolvedEvent,
    email_utils,
)
...
class NotificationsTests(TestCaseBase):
    ...
    @override_settings(
        TIDINGS_CONFIRM_ANONYMOUS_WATCHES=False, STATIC_URL="https://example.com/"
    )
    ...
    def test_autowatch_reply(self, get_current):
        ...
        s = Setting.objects.create(
            user=u, name="questions_watch_after_reply", value="True"
        )
...
class TestAnswerNotifications(TestCaseBase):
    ...
    def test_notify_asker(self):
        ...
        eq_(
            u'{0} posted an answer to your question "{1}"'.format(
    ...
    def test_notify_asker_reply_to(self):
        ...
        self.makeAnswer()
        # Headers should be compared case-insensitively.
        headers = dict((k.lower(), v) for k, v in mail.outbox[0].extra_headers.items())
        eq_("replyto@example.com", headers["reply-to"])
