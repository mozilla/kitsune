...
class AAQConfigFieldRemovalTests(TestCase):
    def test_aaqconfig_created_without_product_or_is_active(self):
        """AAQConfig can be created without product or is_active fields."""
        config = AAQConfigFactory(title="Test Config")
        self.assertEqual(config.title, "Test Config")
        self.assertFalse(hasattr(config, "product"))
        self.assertFalse(hasattr(config, "is_active"))

    def test_question_product_config_via_psc(self):
