...
class MarketplaceAaqTests(TestCase):
    ...
    def test_invalid_category(self):
        ...
        response = get(
            self.client, "questions.marketplace_aaq_category", args=["invalid-category"]
        )
    ...
    def test_payments_category(self):
        ...
        response = get(
            self.client, "questions.marketplace_aaq_category", args=["payments"]
        )
        eq_(200, response.status_code)
        doc = pq(response.content)
    ...
    def test_account_category(self):
        ...
        response = get(
            self.client, "questions.marketplace_aaq_category", args=["account"]
        )
        eq_(200, response.status_code)
        doc = pq(response.content)
...
class SubmitTicketTests(TestCase):
    ...
    def test_submit_ticket(self, get_zendesk):
        ...
        submit_ticket(
            "a@example.com", "cat", "subject", "description", ["tag1", "tag2"]
        )
