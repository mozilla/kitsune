...
@shared_task
@skip_if_read_only_mode
def cleanup_old_spam(logger: BasicLoggerProtocol = log) -> None:
    ...
    logger.info("Starting cleanup of old spam content.")
...
@shared_task
@skip_if_read_only_mode
def update_weekly_votes(logger: BasicLoggerProtocol = log) -> None:
    from kitsune.questions.models import Question, QuestionVote
    ...
    logger.info(f"Started update of {len(qs_to_update)} questions.")
...
@shared_task
@skip_if_read_only_mode
def auto_archive_old_questions(logger: BasicLoggerProtocol = log) -> None:
    from kitsune.questions.models import Answer, Question
    ...
    if q_ids:
        logger.info(f"Updating {len(q_ids)} questions")
