...
urlpatterns = [
    ...
    url(
        r"^/new/(?P<product_key>[\w\-]+)$", views.aaq_step2, name="questions.aaq_step2"
    ),
    url(
    ...
    url(
        r"^/new/(?P<product_key>[\w\-]+)/(?P<category_key>[\w\-]+)/search$",
        views.aaq_step4,
        name="questions.aaq_step4",
    ),
    url(
        r"^/new/(?P<product_key>[\w\-]+)/(?P<category_key>[\w\-]+)/form$",
        views.aaq_step5,
        name="questions.aaq_step5",
    ),
    # AAQ flow for Marketplace
    url(r"^/marketplace$", views.marketplace, name="questions.marketplace_aaq"),
    url(
        r"^/marketplace/success$",
        views.marketplace_success,
        name="questions.marketplace_aaq_success",
    ),
    url(
        r"^/marketplace/refund$",
        views.marketplace_refund,
        name="questions.marketplace_refund",
    ),
    url(
        r"^/marketplace/developer-request$",
        views.marketplace_developer_request,
        name="questions.marketplace_developer_request",
    ),
    url(
        r"^/marketplace/(?P<category_slug>[\w\-]+)$",
        views.marketplace_category,
        name="questions.marketplace_aaq_category",
    ),
    # TODO: Factor out `/(?P<question_id>\d+)` below
    url(r"^/(?P<question_id>\d+)$", views.question_details, name="questions.details"),
    url(
        r"^/(?P<question_id>\d+)/edit$",
        views.edit_question,
        name="questions.edit_question",
    ),
    url(
        r"^/(?P<question_id>\d+)/edit-details$",
        views.edit_details,
        name="questions.edit_details",
    ),
    url(r"^/(?P<question_id>\d+)/reply$", views.reply, name="questions.reply"),
    url(
        r"^/(?P<question_id>\d+)/delete$",
        views.delete_question,
        name="questions.delete",
    ),
    url(r"^/(?P<question_id>\d+)/lock$", views.lock_question, name="questions.lock"),
    url(
        r"^/(?P<question_id>\d+)/archive$",
    ...
    url(
        r"^/(?P<question_id>\d+)/solve/(?P<answer_id>\d+)$",
        views.solve,
    ...
    url(
        r"^/(?P<question_id>\d+)/screen-share/$",
        views.screen_share,
    ...
    url(
        r"^/(?P<question_id>\d+)/feed$",
        answers_feed_view,
        name="questions.answers.feed",
    ),
    url(
        r"^/tagged/(?P<tag_slug>[\w\-]+)/feed$",
        tagged_feed_view,
    ...
    url(
        r"^/(?P<question_id>\d+)/unwatch$",
        views.unwatch_question,
