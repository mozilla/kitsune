...
def detail(request, slug):
    ...
    awards = (Award.objects.filter(badge=badge).order_by("-created"))[
        : settings.BADGE_MAX_RECENT
    ]

    return render(
        request, "badger/badge_detail.html", dict(badge=badge, award_list=awards,)
    )
...
def awards_by_user(request, username):
    ...
    return render(
        request, "badger/awards_by_user.html", dict(user=user, award_list=awards,)
    )
...
def awards_by_badge(request, slug):
    ...
    return render(
        request, "badger/awards_by_badge.html", dict(badge=badge, awards=awards,)
    )
...
def badges_by_user(request, username):
    ...
    return render(
        request, "badger/badges_by_user.html", dict(user=user, badge_list=badges,)
    )
