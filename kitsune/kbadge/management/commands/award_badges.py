        ...
        for year in [2010, 2011, 2012, 2013]:
            ...
            for user in User.objects.filter(id__in=user_ids):
                if maybe_award_questions_badge(
                    questions_badges.QUESTIONS_BADGES["answer-badge"], year, user
                ):
                    print (
            ...
            for user in User.objects.filter(id__in=user_ids):
                if maybe_award_aoa_badge(aoa_badges.AOA_BADGE, year, user):
                    print (
                        "{year} AoA Badge awarded to {user}".format(
                            year=year, user=user.username
                        )
                    )
