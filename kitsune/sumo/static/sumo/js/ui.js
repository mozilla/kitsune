...
(function ($) {
  ...
  $(document).ready(function () {
    ...
    $("[data-toggle]").each(function () {
      ...
      if ($this.data("toggle-sticky") && targetId) {
        if (Modernizr.localstorage) {
          var targetClasses =
            localStorage.getItem(targetId + ".classes") || "[]";
  ...
  var validate_field_cb = function () {
    ...
    if (_.contains(extras, "email")) {
      ...
      var corrected = Mailcheck.findClosestDomain(domain, [
        "gmail.com",
        "yahoo.com",
        "hotmail.com",
      ]);
      ...
      if (
        corrected &&
        corrected !== domain &&
        !_.contains(ignoreList, $this.val())
      ) {
        var $ignore = $("<a />")
          .attr("href", "#")
          .addClass("ignore-email")
          .text(gettext("No, ignore"));
  ...
  $(document).on(
    "keyup",
    "[data-validate-url] input",
    _.throttle(validate_field_cb, 200)
  );
  $(document).on(
    "change",
    "[data-validate-url] input",
