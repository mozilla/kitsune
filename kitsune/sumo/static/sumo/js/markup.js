...
(function ($, gettext, document) {
  ...
  Marky.SimpleButton.prototype = {
    ...
    handleClick: function (e) {
      ...
      if (
        window.codemirror &&
        window.highlighting &&
        window.highlighting.isEnabled()
      ) {
        selText = editor.somethingSelected()
          ? editor.getSelection()
          : this.defaultText;
      ...
      if (document.selection && document.selection.createRange) {
        ...
        if (this.everyline && -~selText.indexOf("\n")) {
          splitText = this._applyEveryLine(
            this.openTag,
            this.closeTag,
            selText
          );
          ...
          if (range.moveStart) {
            range.moveStart(
              "character",
              -1 * this.openTag.length - selText.length
            );
        ...
        parseInt(textarea.selectionStart) === 0
      ) {
        // Firefox/Safari/Chrome/etc.
      ...
      var me = this,
        ...
        $html = $(
          '<section class="marky">' +
            ...
            gettext("Support article:") +
            "</label></div>" +
            '<input type="text" name="internal" placeholder="' +
            gettext("Enter the name of the article") +
            '" /></li>' +
            ...
            gettext("Enter the URL of the external link") +
            '" /></li>' +
            ...
            '<a href="#cancel" class="kbox-cancel sumo-button secondary-button">' +
            gettext("Cancel") +
      ...
      var me = this,
        ...
        $html = $(
          '<section class="marky">' +
            ...
            '" />' +
            '<button type="submit" class="submit-button search-button" title="' +
            gettext("Search Gallery") +
            '">' +
            gettext("Search Gallery") +
            "</button></form>" +
            ...
            gettext("Show:") +
            "</span>" +
            '<ol><li data-type="image" class="selected">' +
            gettext("Images") +
            "</li>" +
            '<li data-type="video">' +
            gettext("Videos") +
            ...
            gettext("Show media for:") +
            " <select /></div></div>" +
            '<div class="placeholder" /><div class="submit sumo-button-wrap reverse-on-desktop align-end">' +
            '<button class="sumo-button primary-button">' +
            gettext("Insert Media") +
            "</button>" +
            '<a href="' +
            galleryUrl +
            '#upload" class="upload sumo-button secondary-button" target="_blank">' +
            gettext("Upload Media") +
            "</a>" +
            '<a href="#cancel" class="kbox-cancel sumo-button push-left">' +
            gettext("Cancel") +
            "</a></div>" +
            "</section>"
        ),
  ...
  Marky.CannedResponsesButton.prototype = $.extend(
    {},
    Marky.SimpleButton.prototype,
    {
      // Gets the DOM node for the button.
        ...
        var me = this,
          ...
          $html = $(
            '<section class="marky">' +
              ...
              '<li class="status-indicator busy">' +
              gettext("Loading...") +
              "</li>" +
              "</ul></div>" +
              ...
              '<h2 class="heading-label preview-label">' +
              gettext("Response editor") +
              "</h2>" +
              '<div class="sumo-button-wrap">' +
              '<button class="toggle-view sumo-button">' +
              gettext("Switch to preview mode") +
              "</button>" +
              "</div>" +
              ...
              '<button id="insert-response" class="sumo-button primary-button">' +
              gettext("Insert Response") +
              "</button>" +
              '<a href="#cancel" class="sumo-button secondary-button kbox-cancel">' +
              gettext("Cancel") +
              "</a>" +
              "</div>" +
              "</section>"
          ...
          $.ajax({
            ...
            success: function (html) {
              ...
              $categories.each(function () {
                var label = $(this).text(),
                  ...
                  $responses = $(document.createElement("ul"))
                ...
                $headingItem
                  ...
                  .on("click", function () {
                    ...
                    $otherHeadings = $categoryList
                      .find(".response-heading")
                      .not($headingItem);
            ...
            error: function () {
              ...
              statusIndicator.text(
                gettext("There was an error checking for canned responses.")
              );
            },
          });
