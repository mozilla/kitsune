...
(function ($) {
  ...
  CONSTANTS.messages.server = gettext(
  ...
  var GalleryUpload = {
    ...
    init: function () {
      ...
      $('input[type="file"]', self.$modal).each(function () {
        ...
        $file.ajaxSubmitInput({
          ...
          onComplete: function ($input, iframeContent, options) {
            $input.closest("form").trigger("ajaxComplete")[0].reset();
      ...
      jQuery.fn.makeCancelUpload = function (options) {
        ...
        var $form = $input
          .wrap('<form class="inline" method="POST" ' + 'action=""/>')
          .closest("form");
      ...
      self.$modal
        .find(".metadata input,.metadata textarea")
    ...
    uploadSuccess: function ($input, iframeJSON, filename) {
      ...
      if (type === "file") {
        ...
        $content = $("<img/>")
          ...
          .wrap('<div class="preview-' + type + '"/>')
          .parent();
      }
      $preview_area = $(".preview." + type, $form);
      $preview_area.filter(".row-right").html($content);
      // Create cancel button.
      attrs["data-action"] = $cancel_btn.data("action") + "?field=" + type;
    ...
    setInputError: function ($input, reason) {
      ...
      $row_input.find("div.details").addClass("error").html(message);
    },
    /*
    ...
    _reUpload: function ($form, type) {
      ...
      if (
        !$form.find(".progress").hasClass("on") &&
