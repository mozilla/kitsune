...
(function ($) {
  ...
  function initVocab() {
    $("div.tags[data-tag-vocab-json]").each(function () {
  ...
  function initAutoComplete() {
    ...
    function makeButtonTender($addForm) {
      ...
      function tendAddButton() {
        ...
        $adder.attr(
          "disabled",
          !tagName.length || isOnscreen || (!canCreateTags && !inVocab)
        );
      }
    ...
    function makeVocabCallback($tags) {
      ...
      function vocabCallback(request, response) {
        var appliedTags = getAppliedTags($tagList),
          vocabMinusApplied = $.grep(vocab, function (e, i) {
    ...
    $("input.autocomplete-tags").each(function () {
      var $input = $(this),
  ...
  function initTagRemoval() {
    ...
    // is always called:
    $("form.remove-tag-form").submit(function () {
      return false;
    });
  }
  ...
  function attachRemoverHandlerTo($container, async) {
    $container.find(".remover").click(function () {
      var $remover = $(this),
  ...
  function addTag($container, async) {
    ...
    function putTagOnscreen(name) {
      ...
      if (!tagIsOnscreen(name, $tagList)) {
        var $li = $(
          "<li class='tag'><span class='tag-name' /><input type='submit' value='&#x2716;' class='remover' /></li> "
        );
        if (async) {
    ...
    if (tagIndex === -1) {
      if (async) {
        // If we're operating wholly client side until Submit is clicked, it would be weird to pretend you've added to the server-side vocab.
        vocab.push(tagName);
      }
    } else {
      // Canonicalize case.
  ...
  function getAppliedTags($tagList) {
    var tagNames = [];
    $tagList.find(".tag .tag-name").each(function (i, e) {
