...
(function () {
  var GeoIPUrl =
    "https://location.services.mozilla.com/v1/country?key=fa6d7fc9-e091-4be1-b6c1-5ada5815ae9d";
  var countryData = {
  ...
  if (suggestedLocale) {
    ...
    $.ajax({
      ...
      .done(function (data) {
        ...
        $message.append(
          $('<button class="sumo-button button-sm confirm" />').text(
            data[currentLocale].confirm
          )
        );
        $message.append(
          $('<button class="sumo-button button-sm cancel" />').text(
            data[currentLocale].cancel
          )
        );

        if (
          data[currentLocale].suggestion !== data[suggestedLocale].suggestion
        ) {
          var $localisedMessage = $("<p />").appendTo($announceBar);
          $localisedMessage.text(suggestedLocaleSuggestion);
          $localisedMessage.append(
            $('<button class="sumo-button button-sm confirm" />').text(
              data[suggestedLocale].confirm
            )
          );
          $localisedMessage.append(
            $('<button class="sumo-button button-sm cancel" />').text(
              data[suggestedLocale].cancel
            )
          );
        }
