...
(function () {
  ...
  Graph.prototype.rebucket = function () {
    ...
    for (i = 0; i < this.data.series.length; i += 1) {
      ...
      if (!series.disabled) {
        this.axisGroups[name].max = Math.max(
          this.axisGroups[name].max,
          series.max
        );
      }
    }
  ...
  Graph.prototype.initBucketUI = function () {
    ...
    var $bucket = $('<div class="bucket"></div>').appendTo(
      this.dom.elem.find(".inline-controls")
    );
    var $select = $("<select>");
  ...
  Graph.prototype.initDateRange = function () {
    ...
    if (this.options.daterange) {
      label_html = interpolate(
        gettext("From %(from_input)s to %(to_input)s"),
        {
  ...
  Graph.prototype.initLegend = function () {
    if (this.options.legend === "mini") {
      ...
      $legend = $('<div class="legend"></div>').appendTo(
        this.dom.elem.find(".inline-controls")
      );
      $series = $("<ul>");
  ...
  Rickshaw.Graph.ScaledBarHoverDetail = Rickshaw.Class.create(
  ...
  Rickshaw.Graph.Axis.ScaledY = function (args) {
    ...
    if (this.side === "left") {
      $(this.graph.element).parent().before($axis).css("margin-left", "10px");
    } else {
      $(this.graph.element).parent().after($axis).css("margin-right", "10px");
    }
  ...
  Rickshaw.Graph.Axis.ScaledY.prototype.render = function () {
    ...
    if (this) {
      ...
      group = svg.append("svg:g").attr("class", "y_ticks plain").call(axis);
      if (this.side === "left") {
        group.attr("transform", "translate(40, 0)");
      } else {
        group
          .select("path.domain")
          .attr("style", "transform: translate(1px, 0)");
      }
    }
  };
  ...
  Rickshaw.Graph.KHover = Rickshaw.Class.create({
    ...
    render: function (args) {
      ...
      for (i = 0; i < args.points.length; i += 1) {
        ...
        li.innerHTML = interpolate(
          '<span class="color" style="background-color: %s;"></span>%s: %s',
