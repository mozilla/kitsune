...
(function ($) {
  ...
  function initReadyForL10n() {
    ...
    $watchDiv.find("a.markasready").click(function () {
      ...
      $("#ready-for-l10n-modal span.revtime").html(
        "(" + $check.data("revdate") + ")"
      );
    });

    $(
      "#ready-for-l10n-modal input[type=submit], #ready-for-l10n-modal button[type=submit]"
    ).click(function () {
      var csrf = $(
        "#ready-for-l10n-modal input[name=csrfmiddlewaretoken]"
      ).val();
  ...
  function initFormLock() {
    ...
    if ($doc.is(".locked")) {
      var $inputs = $doc
        .find("input:enabled, textarea:enabled")
        .prop("disabled", true);
    }
  ...
  function initToggleDiff() {
    ...
    if ($diff.length > 0) {
      $contentOrDiff.append($diff.clone()).append(
        $("<a/>")
    ...
    $draftButton.click(function () {
      ...
      $.post(url, totalData)
        .done(function () {
          var time = new Date(),
            message = interpolate(
              gettext("<strong>Draft has been saved on:</strong> %s"),
              [time]
            );
