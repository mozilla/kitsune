...
(function ($) {
  function init() {
    ...
    if ($body.is(".answers")) {
      ...
      $("#support-search input[name=q]").val(
        k.unquote($.cookie("last_search"))
      );
    ...
    Marky.createSimpleToolbar(".editor-tools", "#reply-content, #id_content", {
      cannedResponses: !$body.is(".new-question"),
    });
    ...
    $("#product-selector select").on("change", function () {
      ...
      document.location =
        document.location.pathname + "?" + $.param(queryParams);
    });
  ...
  function addReferrerAndQueryToVoteForm() {
    ...
    $("form.helpful, .me-too form")
      .append(
        $('<input type="hidden" name="referrer"/>').attr("value", referrer)
      )
  ...
  function initHelpfulVote() {
    $(".sumo-l-two-col--sidebar, #document-list, .answer-tools").each(
      function () {
  ...
  function linkCrashIds(container) {
    ...
    var crashIDRegex = new RegExp(
      "(bp-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})",
      "g"
    );
    var crashStatsBase = "https://crash-stats.mozilla.com/report/index/";
    var helpingWithCrashesArticle = "/kb/helping-crashes";
    var iconPath =
      $("body").data("static-url") + "sumo/img/questions/icon.questionmark.png";
    ...
    container.html(
      container.html().replace(crashIDRegex, crashReportContainer)
    );
  }
  ...
  function initCrashIdLinking() {
    var postContents = $(
      ".question .main-content, .answer .main-content, #more-system-details"
    );
