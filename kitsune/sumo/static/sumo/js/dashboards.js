...
(function ($) {
    ...
    $(".readout-modes").each(function attachClickHandler() {
      ...
      $modes.find(".mode").each(function () {
        ...
        $button.click(function switchMode() {
          ...
          $.get($button.attr("data-url"), function succeed(html) {
  ...
  function initWatchMenu() {
    ...
    $watchDiv.find("input[type=checkbox]").click(function post() {
      ...
      $.ajax({
        ...
        error: function () {
          $box.attr("checked", isChecked ? "" : "checked").attr("disabled", "");
        },
      });
    });
  }
      ...
      $.ajax({
        ...
        statusCode: {
          200: function (data) {
            ...
            var $success = $("#create-announcement")
              .children(".success")
              .show()
              .css({ opacity: 1 });
  ...
  function initL10nStringsStats() {
    ...
    $.getJSON("/media/uploads/l10n_summary.json?_cache=" + cacheBust, function (
      data
    ) {
