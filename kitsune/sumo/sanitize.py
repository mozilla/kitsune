...
patterns used across the kitsune codebase, replacing direct bleach usage.
"""
...
def build_url_policy(allowed_attributes: Mapping[str, Collection[str]]) -> UrlPolicy:
    """Build a UrlPolicy with rules for all URL-like attributes in the allowlist.

    The justhtml package requires explicit (tag, attr) rules for URL-bearing attributes.
    Without a rule, the attribute is always stripped. This function scans the given
    "allowed_attributes" dict and creates permissive rules for every URL-like
    attribute found, matching bleach's behavior of not filtering URLs.
    """
    rules = {}
...
def linkify(text: str, nofollow: bool = False) -> str:
    """Convert plain-text URLs in text to clickable links.

    When nofollow=True, adds rel="nofollow" to generated links.
    Does not sanitize the HTML (linkify only).
    """
    transforms = [Linkify()]
    if nofollow:
        transforms.append(SetAttrs("a", rel="nofollow"))
    doc = JustHTML(text, fragment=True, sanitize=False, transforms=transforms)
    return doc.to_html(pretty=False)
