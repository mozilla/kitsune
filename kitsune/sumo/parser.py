...
def wiki_to_html(
    wiki_markup, locale=settings.WIKI_DEFAULT_LANGUAGE, nofollow=True, tags=None
):
    """Wiki Markup -> HTML"""
...
class WikiParser(Parser):
    ...
    def add_youtube_embeds(self, html):
        ...
        for video_id in self.youtube_videos:
            html = html.replace(
                YOUTUBE_PLACEHOLDER % video_id, generate_youtube_embed(video_id)
            )
        return html
    ...
    def _hook_internal_link(self, parser, space, name):
        ...
        extra_a_attr = ""
        if not link["found"]:
            extra_a_attr += u' class="new" title="{tooltip}"'.format(
    ...
    def _hook_image_tag(self, parser, space, name):
        ...
        title, params = build_hook_params(
            name, self.locale, IMAGE_PARAMS, IMAGE_PARAM_VALUES
        )
...
def generate_youtube_embed(video_id):
    ...
    return render_to_string(
        "wikiparser/hook_youtube_embed.html", {"video_id": video_id}
    )
