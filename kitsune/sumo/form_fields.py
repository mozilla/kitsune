...
class MultiUsernameOrGroupnameField(forms.Field):
    ...
    def to_python(self, value):
        ...
        for key, val in key_value_pairs:
            ...
            if key.lower() == "user" and not User.objects.filter(username=val).exists():
                raise ValidationError(_("{name} is not a valid username.").format(name=val))
            to_objects.setdefault(f"{key.lower()}s", []).append(val)
