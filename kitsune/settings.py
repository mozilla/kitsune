...
# Retrieval (RAG) embeddings. No default backend: an unset value fails closed
# (ImproperlyConfigured) so dev/staging/prod can't silently embed with the fake backend.
# Local and CI set the backend explicitly (.env-dist / .env-test).
RETRIEVAL_EMBEDDING_BACKEND = config("RETRIEVAL_EMBEDDING_BACKEND", default="")
...
RETRIEVAL_MAX_PAGE_OFFSET = config("RETRIEVAL_MAX_PAGE_OFFSET", default=80, cast=int)
...
if SHOW_DEBUG_INFO:
    ...
    # As early as possible, for accurate timings, but after NUL stripping because
    # Silk reads request.GET.
    silk_position = MIDDLEWARE.index("kitsune.sumo.middleware.StripNulCharactersMiddleware") + 1
    MIDDLEWARE = (
        *MIDDLEWARE[:silk_position],
...
ZENDESK_OAUTH_CLIENT_ID = config("ZENDESK_OAUTH_CLIENT_ID", default="")
ZENDESK_OAUTH_CLIENT_SECRET = config("ZENDESK_OAUTH_CLIENT_SECRET", default="")
...
ZENDESK_URGENCY_FIELD_ID = config("ZENDESK_URGENCY_FIELD_ID", default="52638447283860", cast=int)
...
REVIEW_GRACE_PERIOD = config("REVIEW_GRACE_PERIOD", default=168, cast=int)  # Hours - 7 days
