...
DEFAULT_FROM_EMAIL = config(
    "DEFAULT_FROM_EMAIL", default="notifications@support.mozilla.org"
)
DEFAULT_REPLY_TO_EMAIL = config(
    "DEFAULT_REPLY_TO_EMAIL", default="no-reply@mozilla.org"
)
...
if READ_ONLY:
    ...
    if OIDC_ENABLE:
        OIDC_OP_AUTHORIZATION_ENDPOINT = config(
            "OIDC_OP_AUTHORIZATION_ENDPOINT", default=""
        )
        ...
        FXA_OP_AUTHORIZATION_ENDPOINT = config(
            "FXA_OP_AUTHORIZATION_ENDPOINT", default=""
        )
        FXA_OP_USER_ENDPOINT = config("FXA_OP_USER_ENDPOINT", default="")
        FXA_OP_SUBSCRIPTION_ENDPOINT = config(
            "FXA_OP_SUBSCRIPTION_ENDPOINT", default=""
        )
        ...
        FXA_STORE_ACCESS_TOKEN = config(
            "FXA_STORE_ACCESS_TOKEN", default=False, cast=bool
        )
...
CELERY_ALWAYS_EAGER = config(
    "CELERY_ALWAYS_EAGER", default=DEBUG, cast=bool
)  # For tests. Set to False for use.
if not CELERY_ALWAYS_EAGER:
    BROKER_URL = config("BROKER_URL")

CELERY_SEND_TASK_ERROR_EMAILS = config(
    "CELERY_SEND_TASK_ERROR_EMAILS", default=True, cast=bool
)
CELERYD_LOG_LEVEL = config(
    "CELERYD_LOG_LEVEL", default="INFO", cast=lambda x: getattr(logging, x)
)
...
CELERYD_HIJACK_ROOT_LOGGER = config(
    "CELERYD_HIJACK_ROOT_LOGGER", default=False, cast=bool
)
...
BITLY_API_URL = config(
    "BITLY_API_URL", default="http://api.bitly.com/v3/shorten?callback=?"
)
...
TIDINGS_FROM_ADDRESS = config(
    "TIDINGS_FROM_ADDRESS", default="notifications@support.mozilla.org"
)
...
AXES_BEHIND_REVERSE_PROXY = config(
    "AXES_BEHIND_REVERSE_PROXY", default=not DEBUG, cast=bool
)
AXES_REVERSE_PROXY_HEADER = config(
    "AXES_REVERSE_PROXY_HEADER", default="HTTP_X_CLUSTER_CLIENT_IP"
)
...
DMS_GENERATE_MISSING_SHARE_LINKS = config(
    "DMS_GENERATE_MISSING_SHARE_LINKS", default=None
)
...
DMS_UPDATE_L10N_COVERAGE_METRICS = config(
...
DMS_REMOVE_EXPIRED_REGISTRATION_PROFILES = config(
    "DMS_REMOVE_EXPIRED_REGISTRATION_PROFILES", default=None
)
...
DMS_CACHE_MOST_UNHELPFUL_KB_ARTICLES = config(
    "DMS_CACHE_MOST_UNHELPFUL_KB_ARTICLES", default=None
)
DMS_RELOAD_QUESTION_TRAFFIC_STATS = config(
    "DMS_RELOAD_QUESTION_TRAFFIC_STATS", default=None
)
...
DMS_UPDATE_L10N_CONTRIBUTOR_METRICS = config(
    "DMS_UPDATE_L10N_CONTRIBUTOR_METRICS", default=None
)
...
DISABLE_HOSTNAME_MIDDLEWARE = config(
    "DISABLE_HOSTNAME_MIDDLEWARE", default=False, cast=bool
)

DISABLE_FEEDS = config("DISABLE_FEEDS", default=False, cast=bool)
DISABLE_QUESTIONS_LIST_GLOBAL = config(
    "DISABLE_QUESTIONS_LIST_GLOBAL", default=False, cast=bool
)
DISABLE_QUESTIONS_LIST_ALL = config(
    "DISABLE_QUESTIONS_LIST_ALL", default=False, cast=bool
)
IMAGE_ATTACHMENT_USER_LIMIT = config(
    "IMAGE_ATTACHMENT_USER_LIMIT", default=50, cast=int
)

# list of strings to match against user agent to block
USER_AGENT_FILTERS = config("USER_AGENT_FILTERS", default="", cast=Csv())

BADGE_LIMIT_ARMY_OF_AWESOME = config(
    "BADGE_LIMIT_ARMY_OF_AWESOME", default=50, cast=int
)
