...
class EditGroupProfileTests(TestCase):
    ...
    def _verify_get_and_post(self):
        ...
        # Verify POST
        r = self.client.post(
            reverse("groups.edit", locale="en-US", args=[slug]),
...
class EditAvatarTests(TestCase):
    ...
    def test_upload_avatar(self):
        ...
        url = reverse(
            "groups.edit_avatar", locale="en-US", args=[self.group_profile.slug]
        )
        with open("kitsune/upload/tests/media/test.jpg") as f:
    ...
    def test_delete_avatar(self):
        ...
        url = reverse(
            "groups.delete_avatar", locale="en-US", args=[self.group_profile.slug]
        )
...
class AddRemoveMemberTests(TestCase):
    ...
    def test_add_member(self):
        url = reverse(
            "groups.add_member", locale="en-US", args=[self.group_profile.slug]
        )
        r = self.client.get(url)
...
class AddRemoveLeaderTests(TestCase):
    ...
    def test_add_leader(self):
        url = reverse(
            "groups.add_leader", locale="en-US", args=[self.group_profile.slug]
        )
        r = self.client.get(url)
        eq_(405, r.status_code)
