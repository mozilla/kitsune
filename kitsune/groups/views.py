...
from kitsune.groups.forms import GroupProfileForm, GroupAvatarForm, AddUserForm
...
def remove_member(request, group_slug, user_id):
    ...
    return render(
        request, "groups/confirm_remove_member.html", {"profile": prof, "member": user}
    )
...
def remove_leader(request, group_slug, user_id):
    ...
    return render(
        request, "groups/confirm_remove_leader.html", {"profile": prof, "leader": user}
    )
...
def _user_can_edit(user, group_profile):
    ...
    return (
        user.has_perm("groups.change_groupprofile")
        or user in group_profile.leaders.all()
    )
