...
users_patterns = [
    ...
    url(
        r"^/activate/(?P<user_id>\d+)/(?P<activation_key>\w+)$",
        views.activate,
        name="users.activate",
    ),
    ...
    url(r"^/pwresetsent$", views.password_reset_sent, name="users.pw_reset_sent"),
    url(
        r"^/pwreset/(?P<uidb36>[-\w]+)/(?P<token>[-\w]+)$",
        views.password_reset_confirm,
        name="users.pw_reset_confirm",
    ),
    url(
        r"^/pwresetcomplete$",
    ...
    # Forgot username
    url(r"^/forgot-username$", views.forgot_username, name="users.forgot_username"),
    # Change password
    ...
    url(
        r"^confirm_email/(?P<activation_key>\w+)$",
        redirect_to,
...
if settings.OIDC_ENABLE:
    urlpatterns += [
        url(
            r"^fxa/callback/$",
            never_cache(OIDCAuthenticationCallbackView.as_view()),
