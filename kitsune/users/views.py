...
def login(request):
    ...
    if request.user.is_authenticated():
        ...
        res.set_cookie(
            settings.SESSION_EXISTS_COOKIE, "1", secure=False, max_age=max_age
        )
        return res
...
def activate(request, template, activation_key, user_id=None):
    ...
    return render(
        request, template, {"account": account, "questions": my_questions, "form": form}
    )
...
def resend_confirmation(request, template):
    ...
    return render(request, template + "resend_confirmation.html", {"form": form})
...
def profile(request, username):
    ...
    if not (
        request.user.has_perm("users.deactivate_users") or user_profile.user.is_active
    ):
        raise Http404("No Profile matches the given query.")
...
def deactivation_log(request):
    ...
    return render(
        request, "users/deactivation_log.html", {"deactivations": deactivations}
    )
...
def edit_settings(request, template):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request, messages.INFO, _(u"Your settings have been saved.")
            )
...
def edit_avatar(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            content = _create_image_thumbnail(
                user_profile.avatar.path, settings.AVATAR_SIZE, pad=True
            )
    ...
    return render(
        request, "users/edit_avatar.html", {"form": form, "profile": user_profile}
    )
...
def delete_avatar(request):
    ...
    return render(
        request, "users/confirm_avatar_delete.html", {"profile": user_profile}
    )
...
def forgot_username(request, template):
    ...
    if request.method == "POST":
        ...
        if was_valid:
            try_send_email_with_form(
                form.save, form, "email", use_https=request.is_secure()
            )
        ...
        if form.is_valid() or not was_valid:
            ...
            messages.add_message(
                ...
                messages.INFO,
                _(
                    u"We've sent an email with the username to any account"
                    u" using {email}."
                ).format(email=form.data["email"]),
            )
