...
from kitsune.sumo.utils import BasicLoggerProtocol
...
@shared_task
@skip_if_read_only_mode
def cleanup_old_account_events(logger: BasicLoggerProtocol = log) -> None:
    ...
    logger.info(f"Successfully deleted {deleted_count} old account events")


@shared_task
@skip_if_read_only_mode
def cleanup_expired_users(logger: BasicLoggerProtocol = log) -> None:
    ...
    if not waffle.switch_is_active("cleanup-expired-users"):
        logger.info("The cleanup of expired users is not enabled.")
        return
    ...
    logger.info(f"Found {expired_users.count()} expired users")

    for user in expired_users:
        delete_user_pipeline(user)
        logger.info(f"Deleted user {user.username}")

    logger.info(f"Successfully processed {expired_users.count()} expired users")
