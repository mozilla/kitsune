...
class ProfileViewSet(
    ...
    def generate(self, request, **kwargs):
        ...
        for _ in range(10):
            ...
            return Response({"error": "Unable to generate username."}, status=500)
        ...
        return Response(
            {"user": serializer.data, "password": password, "token": token.key,}
        )
    ...
    def delete_setting(self, request, user__username=None):
        ...
        try:
            meta = Setting.objects.get(user=profile.user, name=request.data["name"])
            meta.delete()
