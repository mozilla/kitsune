...
from kitsune.users.models import Profile, RegistrationProfile, RegistrationManager
...
class LoginTests(TestCaseBase):
    ...
    def setUp(self):
        ...
        self.profile_url = (
            reverse(
                "users.profile", args=[self.u.username], locale=settings.LANGUAGE_CODE
            )
            + "?fpa=1"
        )
    ...
    def test_login_password_disabled(self):
        ...
        response = self.client.post(
            reverse("users.login"),
            {"username": self.u.username, "password": "testpass"},
        )
        eq_(200, response.status_code)
    ...
    def test_login_next_parameter(self):
        ...
        response = self.client.post(
            reverse("users.login"),
            {"username": self.u.username, "password": "testpass", "next": next},
        )
        eq_(302, response.status_code)
    ...
    def test_login_invalid_next_parameter(self):
        ...
        response = self.client.post(
            reverse("users.login"),
            {"username": self.u.username, "password": "testpass", "next": invalid_next},
        )
        eq_(302, response.status_code)
        eq_(self.profile_url, response["location"])
...
class PasswordResetTests(TestCaseBase):
    ...
    def test_bad_email(self):
        r = self.client.post(reverse("users.pw_reset"), {"email": "foo@bar.com"})
        eq_(302, r.status_code)
        eq_("/en-US/users/pwresetsent", r["location"])
        eq_(0, len(mail.outbox))

    def test_success(self):
        r = self.client.post(reverse("users.pw_reset"), {"email": self.u.email})
        eq_(302, r.status_code)
        eq_("/en-US/users/pwresetsent", r["location"])
    ...
    def _get_reset_url(self):
        return reverse("users.pw_reset_confirm", args=[self.uidb36, self.token])
    ...
    def test_reset_fail(self):
        ...
        r = self.client.post(
            url, {"new_password1": "onetwo12", "new_password2": "twotwo22"}
        )
        eq_(200, r.status_code)
        doc = pq(r.content)
        eq_("The two password fields didn't match.", doc("ul.errorlist li").text())
    ...
    def test_reset_success(self):
        ...
        r = self.client.post(url, {"new_password1": new_pw, "new_password2": new_pw})
        eq_(302, r.status_code)
...
class EditAvatarTests(TestCaseBase):
    ...
    def test_large_avatar(self):
        ...
        doc = pq(r.content)
        eq_(
            '"test.jpg" is too large (12KB), the limit is 1KB', doc(".errorlist").text()
        )
    ...
    def test_avatar_extensions(self):
        ...
        assert (
            doc(".errorlist")
            .text()
            .startswith("File extension 'ext' is not allowed. Allowed extensions are:")
        )
...
class ViewProfileTests(TestCaseBase):
    ...
    def test_bio_links_no_img(self):
        # bug 1427813
        self.profile.bio = (
            '<p>my dude image <img src="https://www.example.com/the-dude.jpg"></p>'
        )
        self.profile.save()
        r = self.client.get(reverse("users.profile", args=[self.u.username]))
        eq_(200, r.status_code)
...
class ResendConfirmationTests(TestCaseBase):
    ...
    def test_resend_confirmation(self, get_current):
        ...
        r = self.client.post(
            reverse("users.resend_confirmation"), {"email": "testuser@email.com"}
        )
        eq_(200, r.status_code)
