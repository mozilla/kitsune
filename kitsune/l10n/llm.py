...
from kitsune.l10n.models import MachineTranslationRecord
...
def record_localization(doc, content_attribute, target_locale, llm, prompt, response):
    ...
    service = (
        MachineTranslationRecord.SERVICE_OPENAI_API
        if is_openai_model(llm.model_name)
        else MachineTranslationRecord.SERVICE_VERTEX_AI_API
    )

    return MachineTranslationRecord.objects.create(
        service=service,
...
def get_chat_model(model_name):
    ...
    if is_openai_model(model_name):
        ...
        kwargs.update(project="moz-fx-sumo-prod")
