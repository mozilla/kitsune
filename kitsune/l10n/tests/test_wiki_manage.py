...
class ManageExistingMachineTranslationsTests(TestCase):
    ...
    def test_manage_existing_machine_translations_with_no_argument(self):
        ...
        self.assertEqual(
            report["el"]["rejections"],
            [self.rev1_el.get_absolute_url(), self.rev1_el_2.get_absolute_url()],
        )
        self.assertEqual(
            report["el"]["pre_review_approvals"],
            [self.rev2_el.get_absolute_url(), self.rev2_el_2.get_absolute_url()],
        )
        self.assertEqual(
            set(report["ja"].keys()),
            set(["rejections", "pre_review_approvals"]),
        ...
        self.assertEqual(
            report["ja"]["rejections"],
            [self.rev1_ja.get_absolute_url(), self.rev1_ja_2.get_absolute_url()],
        )
        self.assertEqual(
            report["ja"]["pre_review_approvals"],
            [self.rev2_ja.get_absolute_url(), self.rev2_ja_2.get_absolute_url()],
        )
        ...
        self.assertEqual(
            ...
            [self.rev1_ro.get_absolute_url(), self.rev1_ro_2.get_absolute_url()],
        )
