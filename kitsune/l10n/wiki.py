...
from kitsune.l10n.models import ActionRecord
...
def create_machine_translation(model_name, default_doc, target_locale, creator, team_grace_period):
    ...
    ActionRecord.objects.create(
        revision=rev,
        action=(
            ActionRecord.MT_CREATED_AS_APPROVED
            if publish_now
            else ActionRecord.MT_CREATED_AS_AWAITING_REVIEW
        ),
    )
    return rev
...
def manage_existing_machine_translations(mt_config, doc=None):
    ...
    for rev in qs.filter(
        ...
        rev.save()
        # Record the action.
        ActionRecord.objects.create(revision=rev, action=ActionRecord.MT_REJECTED)
    ...
    for rev in (
        ...
        if rev.reviewed is None:
            ...
            rev.save()
            # Record the action.
            ActionRecord.objects.create(revision=rev, action=ActionRecord.MT_APPROVED_PRE_REVIEW)
            ...
            # Record the action.
            ActionRecord.objects.create(
                revision=copy, action=ActionRecord.MT_CREATED_AS_APPROVED_POST_REJECTION
            )
