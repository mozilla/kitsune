...
def get_prior_translation(doc, content_attribute, target_locale):
    """
    ...
    if not (trans_doc and (target := trans_doc.current_revision) and (source := target.based_on)):
        return None

    return dict(
        source_text=getattr(source, content_attribute),
        target_text=getattr(target, content_attribute),
    )
...
def get_messages(
    ...
    """
    A generic function for returning a list of LLM messages ("system" and "user"
    messages)
    """
    context = dict(
...
def get_prompt(doc, content_attribute, target_locale):
    ...
    if content_attribute == "title":
        ...
        prior_translation = get_prior_translation(doc, content_attribute, target_locale)
