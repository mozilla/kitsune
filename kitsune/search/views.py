...
def simple_search(request):
    ...
    * document type (`w=2`, for example, for Support Forum questions only)
    ...
    if not search_form.is_valid():
        ...
        return cache_control(
            render(
                request,
                t,
                {"advanced": False, "request": request, "search_form": search_form},
            ),
    ...
    if request.IS_JSON:
        ...
        data["products"] = [
            {"slug": p.slug, "title": p.title} for p in data["products"]
        ]
...
def advanced_search(request):
    ...
    if a == "2" or not search_form.is_valid():
        ...
        t = "search/form.html"
        data = {"advanced": True, "request": request, "search_form": search_form}
        ...
        return cache_control(render(request, t, data), settings.SEARCH_CACHE_PERIOD)
    ...
    if cleaned_q:
        query_fields = chain(
            *[
                ...
                for cls in [DocumentMappingType, ThreadMappingType, QuestionMappingType]
            ]
        )
        query = {}
