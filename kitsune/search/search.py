...
# Configure temporary logging for debugging
...
class QuestionSearch(SumoSearch):
    ...
    def make_result(self, hit):
        ...
        logger.info("=== QuestionSearch.make_result called ===")
        print(f"Input hit object: {hit}")
        logger.debug(f"Input hit object: {hit}")
        print(f"Hit meta: {hit.meta}")
        logger.debug(f"Hit meta: {hit.meta}")
        print(f"Hit question_id: {getattr(hit, 'question_id', 'N/A')}")
        logger.debug(f"Hit question_id: {getattr(hit, 'question_id', 'N/A')}")
        print(f"Hit question_title: {getattr(hit, 'question_title', 'N/A')}")
        logger.debug(f"Hit question_title: {getattr(hit, 'question_title', 'N/A')}")
        ...
        logger.info(f"Final QuestionSearch result: {result}")
        print("=== QuestionSearch.make_result completed ===\n")
        logger.info("=== QuestionSearch.make_result completed ===")
        return result
...
class WikiSearch(SumoSearch):
    ...
    def make_result(self, hit):
        ...
        logger.info("=== WikiSearch.make_result called ===")
        print(f"Input hit object: {hit}")
        logger.debug(f"Input hit object: {hit}")
        print(f"Hit meta: {hit.meta}")
        logger.debug(f"Hit meta: {hit.meta}")
        print(f"Hit slug: {getattr(hit, 'slug', 'N/A')}")
        logger.debug(f"Hit slug: {getattr(hit, 'slug', 'N/A')}")
        print(f"Hit title: {getattr(hit, 'title', 'N/A')}")
        logger.debug(f"Hit title: {getattr(hit, 'title', 'N/A')}")
        print(f"Hit summary: {getattr(hit, 'summary', 'N/A')}")
        logger.debug(f"Hit summary: {getattr(hit, 'summary', 'N/A')}")
        content_keys = getattr(hit, "content", {}).keys() if hasattr(hit, "content") else "N/A"
        print(f"Hit content keys: {content_keys}")
        logger.debug(f"Hit content keys: {content_keys}")
        ...
        if not summary and hasattr(hit, "summary"):
            ...
            logger.debug(f"Using hit.summary for locale {self.locale}: {summary}")
        if not summary:
            summary = hit.content[self.locale][:SNIPPET_LENGTH]
            print(f"Using content summary: {summary[:100]}...")
            logger.debug(f"Using content summary: {summary[:100]}...")
        summary = strip_html(summary)
        print(f"Summary after HTML stripping: {summary[:100]}...")
        logger.debug(f"Summary after HTML stripping: {summary[:100]}...")
        ...
        logger.info(f"Final WikiSearch result: {result}")
        print("=== WikiSearch.make_result completed ===\n")
        logger.info("=== WikiSearch.make_result completed ===")
        return result
...
class ProfileSearch(SumoSearch):
    ...
    def make_result(self, hit):
        ...
        logger.info("=== ProfileSearch.make_result called ===")
        print(f"Input hit object: {hit}")
        logger.debug(f"Input hit object: {hit}")
        print(f"Hit meta: {hit.meta}")
        logger.debug(f"Hit meta: {hit.meta}")
        print(f"Hit username: {getattr(hit, 'username', 'N/A')}")
        logger.debug(f"Hit username: {getattr(hit, 'username', 'N/A')}")
        print(f"Hit name: {getattr(hit, 'name', 'N/A')}")
        logger.debug(f"Hit name: {getattr(hit, 'name', 'N/A')}")
        print(f"Hit avatar: {getattr(hit, 'avatar', 'N/A')}")
        logger.debug(f"Hit avatar: {getattr(hit, 'avatar', 'N/A')}")
        print(f"Hit meta.id: {getattr(hit.meta, 'id', 'N/A')}")
        logger.debug(f"Hit meta.id: {getattr(hit.meta, 'id', 'N/A')}")
        ...
        logger.info(f"Final ProfileSearch result: {result}")
        print("=== ProfileSearch.make_result completed ===\n")
        logger.info("=== ProfileSearch.make_result completed ===")
        return result
...
class ForumSearch(SumoSearch):
    ...
    def make_result(self, hit):
        ...
        logger.info("=== ForumSearch.make_result called ===")
        print(f"Input hit object: {hit}")
        logger.debug(f"Input hit object: {hit}")
        print(f"Hit meta: {hit.meta}")
        logger.debug(f"Hit meta: {hit.meta}")
        print(f"Hit thread_title: {getattr(hit, 'thread_title', 'N/A')}")
        logger.debug(f"Hit thread_title: {getattr(hit, 'thread_title', 'N/A')}")
        hit_content = getattr(hit, "content", "N/A")[:100] if hasattr(hit, "content") else "N/A"
        print(f"Hit content: {hit_content}...")
        logger.debug(f"Hit content: {hit_content}...")
        print(f"Hit updated: {getattr(hit, 'updated', 'N/A')}")
        logger.debug(f"Hit updated: {getattr(hit, 'updated', 'N/A')}")
        print(f"Hit forum_slug: {getattr(hit, 'forum_slug', 'N/A')}")
        logger.debug(f"Hit forum_slug: {getattr(hit, 'forum_slug', 'N/A')}")
        print(f"Hit thread_id: {getattr(hit, 'thread_id', 'N/A')}")
        logger.debug(f"Hit thread_id: {getattr(hit, 'thread_id', 'N/A')}")
        ...
        logger.info(f"Final ForumSearch result: {result}")
        print("=== ForumSearch.make_result completed ===\n")
        logger.info("=== ForumSearch.make_result completed ===")
        return result
