...
class SearchMixin(object):
    ...
    * get_mapping_type
    ...
    def index_later(self):
        ...
        _local_tasks().add((index_task.delay, (self.get_mapping_type(), (self.pk,))))

    def unindex_later(self):
        """Register myself to be unindexed at the end of the request."""
        _local_tasks().add((unindex_task.delay, (self.get_mapping_type(), (self.pk,))))
...
class SearchMappingType(MappingType, Indexable):
    ...
    def reshape(cls, results):
        ...
        return [
            dict(
                (key, (val if key in list_keys else val[0]))
