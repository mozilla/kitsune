...
class AdvancedSearchTests(ElasticTestCase):
    ...
    def test_search_products(self):
        ...
        response = self.client.get(
            reverse("search.advanced"), {"a": "1", "product": "product", "q": "cookies", "w": "1"}
        )
    ...
    def test_wiki_no_query(self):
        ...
        response = self.client.get(
            reverse("search.advanced"),
            {"q": "", "tags": "desktop", "w": "1", "a": "1", "format": "json"},
        )
    ...
    def test_questions_sortby(self):
        ...
        response = self.client.get(
            reverse("search.advanced"),
            {"q": "tags", "tags": "desktop", "w": "2", "a": "1", "sortby": "3", "format": "json"},
        )
    ...
    def test_sortby_documents_helpful(self):
        ...
        response = self.client.get(
            reverse("search.advanced"),
            {"w": "1", "a": "1", "sortby_documents": "helpful", "format": "json"},
        )
        eq_(200, response.status_code)
    ...
    def test_forums_search_authorized_forums(self):
        ...
        PermissionFactory(
            codename="forums_forum.view_in_forum", content_type=ct, object_id=forum2.id, group=g
        )
    ...
    def test_category_invalid(self):
        ...
        d1 = DocumentFactory(
            title="d1 audio", locale="en-US", category=10, is_archived=False, tags=["desktop"]
        )
    ...
    def test_created(self):
        ...
        # on 6/21/2010
        q2 = QuestionFactory(
            title="q2 audio", created=(created_ds + timedelta(days=2)), tags=["desktop"]
        )
    ...
    def test_archived(self):
        ...
        qs = {"q": "impalas", "a": 1, "w": 1, "format": "json", "include_archived": "on"}
        response = self.client.get(reverse("search.advanced"), qs)
        results = json.loads(response.content)["results"]
        eq_(1, len(results))
