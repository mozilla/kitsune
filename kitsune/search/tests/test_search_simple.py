...
class SimpleSearchTests(ElasticTestCase):
    ...
    def test_headers(self):
        ...
        response = self.client.get(reverse("search"), {"q": "audio"})
        eq_(
            "max-age=%s" % (settings.SEARCH_CACHE_PERIOD * 60),
    ...
    def test_json_format(self):
        ...
        response = self.client.get(
            reverse("search"), {"q": "bookmarks", "format": "json",}
        )
    ...
    def test_json_callback_validation(self):
        ...
        response = self.client.get(
            reverse("search"),
            {"q": "bookmarks", "format": "json", "callback": "callback",},
        )
    ...
    def test_json_empty_query(self):
        ...
        response = self.client.get(
            reverse("search"), {"format": "json", "a": 0,}, follow=True
        )
        eq_(response["Content-Type"], "application/json")
    ...
    def test_fallback_for_zero_results(self):
        ...
        doc = DocumentFactory(
            title=u"audio1", locale=u"en-US", category=10, products=[firefox]
        )
        RevisionFactory(document=doc, is_approved=True)
        doc = DocumentFactory(
            title=u"audio2", locale=u"en-US", category=10, products=[firefox]
        )
        RevisionFactory(document=doc, is_approved=True)
    ...
    def test_filter_by_doctype(self):
        ...
        doc = DocumentFactory(
            title=u"audio", locale=u"en-US", category=10, products=[desktop]
        )
        RevisionFactory(document=doc, is_approved=True)

        doc = DocumentFactory(
            title=u"audio too", locale=u"en-US", category=10, products=[desktop]
        )
        RevisionFactory(document=doc, is_approved=True)
        ...
        response = self.client.get(
            reverse("search"), {"q": "audio", "format": "json", "w": "1"}
        )
        eq_(200, response.status_code)
        content = json.loads(response.content)
        eq_(content["total"], 2)
        ...
        response = self.client.get(
            reverse("search"), {"q": "audio", "format": "json", "w": "2"}
        )
        eq_(200, response.status_code)
        content = json.loads(response.content)
        eq_(content["total"], 1)
