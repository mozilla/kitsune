...
def get_indexes(all_indexes=False):
    ...
    if not all_indexes:
        indexes = dict(
            (k, v) for k, v in indexes.items() if k.startswith(settings.ES_INDEX_PREFIX)
        )

    return [(name, value["docs"]["num_docs"]) for name, value in indexes.items()]
    ...
    for index in indexes:
        ...
        es.indices.create(
            ...
            body={
                ...
                "settings": {"analysis": get_analysis(),},
            },
        )
...
def es_reindex_cmd(
    ...
    try:
        ...
        for cls, indexable in all_indexable:
            ...
            log.info(
                "reindexing %s. %s to index....", cls.get_mapping_type_name(), total
            )
        ...
        for index, old_refresh in old_refreshes.items():
            es.indices.put_settings(
                index=index, body={"index": {"refresh_interval": old_refresh}}
            )
...
def es_delete_cmd(index, noinput=False, log=log):
    ...
    if index in all_read_indexes() and not noinput:
        ret = raw_input(
            '"%s" is a read index. Are you sure you want '
