...
class AdvancedSearchForm(forms.Form):
    ...
    topics = forms.MultipleChoiceField(
        required=False, widget=forms.CheckboxSelectMultiple(), label=_lazy(u"Topics")
    )

    language = forms.ChoiceField(
        required=False, label=_lazy(u"Language"), choices=SEARCH_LANGUAGES
    )
    ...
    user_widget = forms.TextInput(
        attrs={"placeholder": _lazy(u"username"), "class": "auto-fill"}
    )
    ...
    tag_widget = forms.TextInput(
        attrs={"placeholder": _lazy(u"tag1, tag2"), "class": "auto-fill"}
    )
    q_tags = forms.CharField(label=_lazy(u"Tags"), required=False, widget=tag_widget)
    ...
    def set_allowed_forums(self, user):
        ...
        forums = [
            (f.id, f.name) for f in DiscussionForum.authorized_forums_for_user(user)
        ]
