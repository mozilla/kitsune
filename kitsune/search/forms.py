...
class AdvancedSearchForm(forms.Form):
    ...
    topics = forms.MultipleChoiceField(
        required=False, widget=forms.CheckboxSelectMultiple(), label=_lazy("Topics")
    )

    language = forms.ChoiceField(required=False, label=_lazy("Language"), choices=SEARCH_LANGUAGES)
    ...
    user_widget = forms.TextInput(attrs={"placeholder": _lazy("username"), "class": "auto-fill"})
    ...
    tag_widget = forms.TextInput(attrs={"placeholder": _lazy("tag1, tag2"), "class": "auto-fill"})
    q_tags = forms.CharField(label=_lazy("Tags"), required=False, widget=tag_widget)
    ...
    def __init__(self, *args, **kwargs):
        ...
        topics_field.choices = Topic.objects.values_list("slug", "title").distinct()
    ...
    def clean(self):
        ...
        date_fields = (("created", "created_date"), ("updated", "updated_date"))
        for field_option, field_date in date_fields:
            if c[field_date] != "":
                try:
                    created_timestamp = time.mktime(time.strptime(c[field_date], "%m/%d/%Y"))
    ...
    def set_allowed_forums(self, user):
        ...
        forums = [(f.id, f.name) for f in DiscussionForum.authorized_forums_for_user(user)]
