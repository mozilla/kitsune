...
def ban(request):
    ...
    if not created and account.banned:
        raise GenericAPIException(status.HTTP_409_CONFLICT, "This account is already banned!")
    else:
        account.banned = True
        account.save()
...
def unban(request):
    ...
    if not usernames:
        raise GenericAPIException(status.HTTP_400_BAD_REQUEST, "Usernames not provided.")
