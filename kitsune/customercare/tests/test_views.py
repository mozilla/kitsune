...
class TicketMarkSolvedTests(TestCase):
    """Tests for the ticket_mark_solved view."""
    ...
    def setUp(self):
        ...
        self.client.force_login(self.owner)
        self.url = reverse(
            "customercare.ticket_mark_solved",
            args=[self.owner.username, self.ticket.id],
        )
    ...
    def _audit(self, status="solved", updated_at="2026-05-28T12:00:00Z"):
        ticket_view = SimpleNamespace(
            id=int(self.ticket.zendesk_ticket_id),
            status=status,
            updated_at=updated_at,
        )
        return SimpleNamespace(ticket=ticket_view)
    ...
    @patch("kitsune.customercare.views.ZendeskClient")
    def test_post_persists_status_and_updated_at(self, mock_client_cls):
        mock_client_cls.return_value.update_ticket_status.return_value = self._audit(
            updated_at="2026-05-28T13:00:00Z"
        )
        self.client.post(self.url)
        self.ticket.refresh_from_db()
        self.assertEqual("solved", self.ticket.zd_status)
        self.assertEqual(parse_datetime("2026-05-28T13:00:00Z"), self.ticket.zd_updated_at)

    @patch("kitsune.customercare.views.ZendeskClient")
    def test_htmx_post_emits_oob_status_badge(self, mock_client_cls):
        mock_client_cls.return_value.update_ticket_status.return_value = self._audit()
    ...
    @patch("kitsune.customercare.views.ZendeskClient")
    def test_htmx_post_renders_partial(self, mock_client_cls):
        mock_client_cls.return_value.update_ticket_status.return_value = self._audit()
        response = self.client.post(self.url, HTTP_HX_REQUEST="true")
        self.assertEqual(200, response.status_code)
        self.assertContains(response, 'id="thread-replies"')
    ...
    @patch("kitsune.customercare.views.ZendeskClient")
    def test_post_zendesk_api_exception_sets_status_error(self, mock_client_cls):
        mock_client_cls.return_value.update_ticket_status.side_effect = APIException("nope")
        response = self.client.post(self.url, HTTP_HX_REQUEST="true")
        self.ticket.refresh_from_db()
        self.assertEqual(SupportTicket.ZD_STATUS_OPEN, self.ticket.zd_status)
        self.assertIn(b"mark this ticket as solved", response.content.lower())

    @patch("kitsune.customercare.views.ZendeskClient")
    def test_post_request_exception_sets_status_error(self, mock_client_cls):
    ...
    @patch("kitsune.customercare.views.ZendeskClient")
    def test_post_on_solved_ticket_skips_zendesk(self, mock_client_cls):
        """Already-solved ticket: no Zendesk call, no user-visible error notice."""
        self.ticket.zd_status = SupportTicket.ZD_STATUS_SOLVED
        self.ticket.save(update_fields=["zd_status"])
        response = self.client.post(self.url, HTTP_HX_REQUEST="true")
        self.assertEqual(200, response.status_code)
        mock_client_cls.return_value.update_ticket_status.assert_not_called()
        # The button is gated on ZD_STATUSES_SOLVABLE, so neither it nor the
        # error notice appears in the response — implicit feedback that the
        # action no longer applies.
        self.assertNotIn(b"mark this ticket as solved", response.content.lower())
        self.assertNotIn(b"mark-solved-submit", response.content)

    @patch("kitsune.customercare.views.ZendeskClient")
    def test_post_on_closed_ticket_skips_zendesk(self, mock_client_cls):
        """Closed ticket: no Zendesk call, no user-visible error notice, no button."""
        self.ticket.zd_status = SupportTicket.ZD_STATUS_CLOSED
        self.ticket.save(update_fields=["zd_status"])
        response = self.client.post(self.url, HTTP_HX_REQUEST="true")
        self.assertEqual(200, response.status_code)
    ...
    @patch("kitsune.customercare.views.ZendeskClient")
    def test_post_on_new_state_skips_zendesk(self, mock_client_cls):
        """NEW state is also outside ZD_STATUSES_SOLVABLE: silent no-op."""
    ...
    @patch("kitsune.customercare.views.ZendeskClient")
    def test_post_by_staff_view_access_returns_403(self, mock_client_cls):
        """A staff user with view access but not the owner gets 403 (PermissionDenied)."""
        staff = UserFactory(is_staff=True)
        self.client.force_login(staff)
        response = self.client.post(self.url)
        self.assertEqual(403, response.status_code)
    ...
    def test_template_hides_button_for_non_owner_who_can_view(self):
        """Non-owners with view access do NOT see the Mark as solved button."""
        staff = UserFactory(is_staff=True)
        self.client.force_login(staff)
