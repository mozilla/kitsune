...
export default class Chart {
  constructor($container, options) {
    let defaults = {
      ...
      colorScale() {
        return d3.scale.quantize().domain([0, 100]).range(this.chartColors);
      },
  ...
  preRender() {
    ...
    this.dom.svg = d3
      .select(this.dom.graphContainer)
      .append("svg")
    ...
    this.dom.svg.append("g").attr("class", "data");
  ...
  setupAxis(axis) {
    let axisGroup = this.dom.svg.append("g").attr("class", axis);
    ...
    axisGroup
      ...
      .attr("x", (d, i) => {
        return (
          this.axes.getPosition("x", axis, i) + this.axes[axis].labelOffsets.x
        );
      })
      .attr("y", (d, i) => {
        return (
          this.axes.getPosition("y", axis, i) + this.axes[axis].labelOffsets.y
        );
      });
  }
  ...
  setupLegend() {
    ...
    let legend = this.dom.svg.append("g").attr("class", "legend");
  ...
  populateData(filter) {
    ...
    this.dom.cohorts.each((cohort, i) => {
      ...
      let coloredBoxes = gSelection
        .selectAll("rect")
        .data(cohort.retention_metrics);

      let sizeText = gSelection
        .selectAll("text")
        .data(cohort.retention_metrics);
      ...
      coloredBoxes
        ...
        .style("fill", (d) => {
          return this.colorScale()(
            Math.floor((d.size / cohortOriginalSize) * 100) || 0
          );
        })
        .style("stroke", "#000")
