...
class TestTopContributorsQuestions(ElasticTestCase):
    ...
    def test_it_works(self):
        ...
        eq_(
            data["results"][0]["last_contribution_date"],
        ...
        eq_(
            data["results"][1]["last_contribution_date"],
    ...
    def test_filter_last_contribution(self):
        ...
        req = self.factory.get(
            "/", {"last_contribution_date__gt": yesterday.strftime("%Y-%m-%d")}
        )
        data = self.api.get_data(req)
        ...
        req = self.factory.get(
            "/", {"last_contribution_date__lt": yesterday.strftime("%Y-%m-%d")}
        )
        data = self.api.get_data(req)
