...
def search(request, media_type):
    ...
    return render(
        request,
        "gallery/search.html",
        {"media": media, "media_type": media_type, "q": term},
    )
...
def delete_media(request, media_id, media_type="image"):
    ...
    log.warning(
        "User %s is deleting %s with id=%s" % (request.user, media_type, media.id)
    )
    media.delete()
    # Rebuild KB
    schedule_rebuild_kb()
...
def upload_async(request, media_type="image"):
    ...
    return HttpResponseBadRequest(
        json.dumps(
            {"status": "error", "message": unicode(message), "errors": file_info}
        )
    )
