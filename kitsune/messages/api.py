...
def get_autocomplete_suggestions(pre, is_staff=False):
    ...
    def create_suggestion(item, item_type, is_user=True):
        ...
        return {
            ...
            "display_name": display_name_or_none(item) if is_user else item.name,
...
@require_GET
@json_view
def usernames_or_groups(request):
    ...
    return get_autocomplete_suggestions(pre, request.user.profile.is_staff)
